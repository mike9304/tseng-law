...
function buildBlogCard(post: BlogPost, idx: number): BuilderCanvasNode[] {
  ...
  return [
    ...
    heading(`${cid}-title`, { x: 20, y: 196, width: 330, height: 72 }, post.title, 3, '#2a2418', 'left', cid),
    createTextNode({
      id: `${cid}-date`,
      parentId: cid,
...
const nodes: BuilderCanvasNode[] = assignCanvasNodeZIndices([
  heading('tpl-edublog-title', { x: MARGIN, y: 50, width: 500, height: 56 }, '교육 뉴스', 1, '#2a2418'),
  createTextNode({
  ...
  ...buildCategoryProofSections('education', 'education-blog', STAGE_H),
]);
