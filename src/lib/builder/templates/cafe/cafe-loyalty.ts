...
const tierCards: BuilderCanvasNode[] = tiers.flatMap((t, i) => {
  ...
  const bg = i === 2 ? '#1f2622' : '#efe1c8';
  const textColor = i === 2 ? '#ffffff' : '#1f2622';
  const titleColor = i === 2 ? '#c26f3d' : '#1f2622';
  return [
    createContainerNode({
      id: prefix,
...
const nodes: BuilderCanvasNode[] = assignCanvasNodeZIndices([
  ...
  /* ── Program explanation ────────────────────────────────── */
  heading('tpl-cafeloy-prog-title', { x: 80, y: PROGRAM_Y, width: 400, height: 50 }, '로열티 프로그램 안내', 2, '#1f2622', 'left'),
  createTextNode({
  ...
  /* ── Reward tiers ───────────────────────────────────────── */
  heading('tpl-cafeloy-tiers-title', { x: 80, y: TIERS_Y, width: 400, height: 50 }, '등급별 혜택', 2, '#1f2622', 'left'),
  ...tierCards,
  ...
  ...buildCategoryProofSections('cafe', 'cafe-loyalty', STAGE_H),
]);
