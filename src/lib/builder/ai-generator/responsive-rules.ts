...
import {
  buildNodeOverflowSuggestion,
  buildStackSuggestions,
  clampWidth,
  effectiveRect,
} from '@/lib/builder/ai-generator/responsive-layout-rules';
...
export function scanResponsiveSuggestions(input: ResponsiveScanInput): ResponsiveSuggestion[] {
  ...
  const layoutOverflow: ResponsiveSuggestion[] = [];
  for (const node of nodes) {
    ...
    const overflow = buildNodeOverflowSuggestion(node, viewport, safeWidth);
    if (overflow) layoutOverflow.push(overflow);
  }
  for (const stack of buildStackSuggestions(nodes, viewport, safeWidth, stackGap)) {
    raw.push(stack);
  }
  raw.push(...layoutOverflow);
