...
const GRID_H = 480; // tightened from 640: 2-row grid only needs ~460px, was leaving a large empty gap before the next section
...
const cards: BuilderCanvasNode[] = integrations.flatMap((intg, i) => {
  ...
  return [
    ...
    heading(`${prefix}-t`, { x: 24, y: 24, width: 222, height: 36 }, intg.name, 3, '#16191f', 'left', prefix),
    createTextNode({
      id: `${prefix}-d`,
      parentId: prefix,
...
const nodes: BuilderCanvasNode[] = assignCanvasNodeZIndices([
  ...
  heading('tpl-stupintg-grid-title', { x: 80, y: GRID_Y, width: 400, height: 50 }, '지원 서비스', 2, '#16191f', 'left'),
  ...cards,
  ...buildCategoryProofSections('startup', 'startup-integrations', STAGE_H),
]);
