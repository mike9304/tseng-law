...
- [ ] **D2 x-default 정책**: seo-geo-expert 코퍼스 판정에 따라 x-default /ko→/en 전환 또는 유지 결정·구현. 검증: 로컬 빌드 sitemap.xml + 페이지 alternates 태그 diff.
- [ ] **D3 EN 인텐트 콘텐츠 보강**: ① korean-lawyer-in-taiwan 타이틀 중복 수정 ② expert P레버에 따른 EN 랜딩/가이드 신설·확장(영어권+SEA 질의 클러스터, 직답형 H2/H3+2-3문장, 날짜 실값) ③ JSON-LD 보강(availableLanguage 등 expert 판정분). 광고규정 게이트 준수(승소율·결과보장 금지). 검증: 로컬 렌더 HTML 실측 + qa.
...
- [ ] **D6 QA+커밋+배포 준비**: 전체 qa 그린, 이 브랜치 커밋 정리, main 병합 전략 문서화(visit-metrics 682f3226·로컬 main ahead2와 조율). **push/배포는 사용자 1회 확인 후**.
