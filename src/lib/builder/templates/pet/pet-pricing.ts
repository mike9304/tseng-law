...
const pkgCards: BuilderCanvasNode[] = packages.flatMap((pkg, i) => {
  ...
  const bg = i === 2 ? '#2b2018' : '#f3e7da';
  const textColor = i === 2 ? '#ffffff' : '#2b2018';
  const titleColor = i === 2 ? '#ffffff' : '#2b2018';
  return [
    createContainerNode({
      id: prefix,
    ...
    createButtonNode({
      ...
      style: { backgroundColor: '#d97742', borderRadius: 6 },
    }),
  ];
});
...
const nodes: BuilderCanvasNode[] = assignCanvasNodeZIndices([
  ...
  heading('tpl-petprc-list-title', { x: 80, y: PRICES_Y, width: 400, height: 50 }, '서비스별 진료비', 2, '#2b2018', 'left'),
  ...priceNodes,

  heading('tpl-petprc-pkg-title', { x: 80, y: PACKAGES_Y, width: 400, height: 50 }, '웰니스 패키지', 2, '#2b2018', 'left'),
  ...pkgCards,
  ...buildCategoryProofSections('pet', 'pet-pricing', STAGE_H),
]);
