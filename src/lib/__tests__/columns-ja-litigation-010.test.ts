...
const officialAmount = '157万9,589台湾ドル（TWD 1,579,589）';
...
const mediaRecords = [
  {
    image: 'img-02.jpg',
    caption:
      '男子大学生、90キロのデッドリフトで椎間板を損傷しジムに賠償請求',
  ...
  },
  {
    image: 'img-03.jpg',
    caption:
      '韓国人男子大学生、90キロのデッドリフトで椎間板を損傷―一審で157万台湾ドル、控訴審で和解と報道',
  ...
  },
  {
    image: 'img-06.jpg',
    caption:
      'PTTでのニュース討論：韓国人の男子大学生が90キロのデッドリフトで椎間板を損傷、一審でジムに157万台湾ドルの賠償命令',
    url: 'https://www.ptt.cc/bbs/MuscleBeach/M.1680935985.A.BF6.html',
  },
  {
    image: 'img-07.jpg',
    caption:
      'ブログ記事：体重70キロの韓国人大学生が90キロのデッドリフトで負傷―ジムの責任をどう考えるか',
    url: 'https://blog.udn.com/blackjack/179081715',
  },
  {
    image: 'img-08.jpg',
    caption:
      '法律解説：男子大学生がデッドリフトで椎間板を損傷、一審で有名ジムに157万台湾ドルの賠償命令',
  ...
  },
  {
    image: 'img-09.jpg',
    caption:
      '判決紹介：ジム初心者が90キロのデッドリフトを求められ、急性椎間板損傷を負った事例',
...
describe('Japanese litigation column 010 — gym injury damages', () => {
  ...
  it('derives read time from the exact visible Japanese count and meaningful kana', () => {
    ...
    expect(visibleJapaneseCount).toBe(3_930);
    expect(visibleJapaneseCount).toBeGreaterThanOrEqual(3_500);
    expect(visibleKanaCount).toBe(1_916);
  ...
  it('attributes the exact official first-instance result and appeal report correctly', () => {
    ...
    expect(raw).toContain('当事者が控訴審で和解したとメディアが報じています');
    expect(raw).toContain('公式の一審判決が認定した事実ではありません');
    expect(raw).toContain(
      '同判決から控訴審の結果や和解額を確認することはできません。',
    );
  ...
  it('links every controlling provision once and preserves fact-dependent limits', () => {
    ...
    const requiredRules = [
      ...
      'どのような注意義務があったか、その義務に違反したか、違反と負傷との因果関係、現実に生じた損害、相手方の抗弁、双方の証拠',
      '事実が過失傷害罪の構成要件を満たす場合には、刑事告訴が選択肢となり得ます。',
      '契約責任、不法行為責任、消費者保護法上の責任',
      '複数の手段が考えられるからといって、すべての手続を行うべきであるとは限らず、結果が保証されるわけでもありません。',
  ...
  it('qualifies evidence preservation without promising compulsion or recovery', () => {
    const requiredEvidenceRules = [
      '防犯カメラ映像、診療録や診断書、医療費などの支出を示す領収書、ジムまたはトレーナーとの連絡、目撃者の説明、予約・出席記録、トレーニング計画や実施記録',
      '内容証明郵便や弁護士名義の書面により、ジムへ正式な保存要請をする方法',
      'いつ何の保存を求めたかを記録する実務上の意味',
      '新たな強制的保存義務が当然に生じるわけではなく、資料の削除を確実に防げるものでもありません。',
      '自動的に不利な推認が働くとは限りません。',
      '事実関係が犯罪に当たり得るときは、速やかに捜査機関に被害を申告し、映像を適法に取得・保全する根拠があるかについて判断を求めることが考えられます。',
      '申告をすれば警察が必ず映像を取得するというものではない',
    ];
  ...
  it('covers all permitted damages and keeps insurance and closing qualified', () => {
    const requiredRules = [
      ...
      '**非財産的損害**',
      '**懲罰的損害賠償**',
      '実際に認められるか、またその額は、各損害の発生・必要性、事故との因果関係、証拠、責任割合および裁判所の判断によります。',
      '障害の等級や割合だけで賠償額が決まるわけではなく、退職時までの損失が自動的に計算されるものでもありません。',
      '実損害額の5倍以下、重大な過失について3倍以下、過失について1倍以下',
      '同法がその事案に適用されるか、裁判所が賠償を認めるか、その額をいくらとするかは、法定要件、証拠、裁判所の評価によります。',
      '責任保険の存在だけで、ジムやトレーナーの法的責任が成立したり、支払額が確定したりするわけではありません。',
      '補償範囲、保険金額、免責・除外条項、事故と負傷との因果関係、各損害の必要性や評価',
      '保険による解決や交渉が適しているかは、約款、補償範囲、手元の証拠、期限、当事者の具体的な事情や意向に応じて個別に検討します。',
      '交渉、消費者保護機関への申立てや調停、刑事告訴、民事上の損害賠償請求は、事情に応じて選択し得る手段であり、すべての案件で訴訟を提起したり、これらの手続をすべて行ったりする必要はありません。',
    ];
    for (const rule of requiredRules) expect(raw).toContain(rule);
  });
