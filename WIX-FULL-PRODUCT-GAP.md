...
| Dynamic content | F17-F26 | Datasets, repeaters, dynamic pages | Partial: home insights/services runtime datasets + generic repeater preview contract + scene/layers repeater preview nodes + selected-record URL/SEO preview + page templates + canvas service/FAQ item focus into persisted collection editor + admin canvas service/FAQ repeater quick edit + dynamic template editor v0 block/record preview + draft save/load + publish slot + public route block visibility consumption | Full visual repeater authoring and deeper dynamic layout rendering |
