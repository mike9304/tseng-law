...
| M158 | CMS | CMS collections/content manager v1 | F07~F16 | 24h | M157 | 🟡 F07~F11/F13~F16 green; editable CRUD + duplicate/search/sort + bulk publish/draft/archive/delete + CSV mapping/summary/rollback + named revision diffs/restore + typed filters/saved views/stable pagination + typed field/validation UI + permissions UI/gates are in; F12 actor identity hardening remains |
| M159 | CMS | Dataset/repeater/dynamic page binding | F17~F26 | 32h | M158 | 🟡 home insights + services runtime datasets, generic repeater preview contract, scene/layers repeater preview nodes, selected-record dynamic URL + per-item SEO preview, service dataset inspector/E2E green, canvas service/FAQ item click focuses persisted collection editor, admin canvas service/FAQ repeater quick edit, dynamic template editor v0 block/record preview + draft save/load + publish slot + public route block visibility consumption / full visual repeater authoring remains |
