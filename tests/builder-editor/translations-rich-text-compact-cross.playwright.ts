...
test.describe('/api/builder/translations/edit compact cross rich text persistence', () => {
  ...
  test('preserves compact translated cross separators outside adjacent rich text marks', async ({ request }) => {
    const token = `translation-rich-compact-cross-${Date.now().toString(36)}`;
    const nodeId = `translation-rich-compact-cross-node-${token}`;
