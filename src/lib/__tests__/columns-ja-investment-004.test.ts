...
const incentivePeriodAmountParagraph =
  '現行の「産業創新条例」第10条の1は、2025年1月1日から2029年12月31日までの一定の投資を対象としています。同一課税年度に100万新台湾ドル（NT$）以上NT$20億以下を投資する会社または有限合夥が、法定要件と承認手続のもとで適用を検討することができます。投資家は対象資産を自己使用目的で取得しなければならず、新品であることと実際の使用状況も確認しなければなりません。';
...
const section6Heading = '## 6. 台湾・韓国所得税協定と恒久的施設（PE）';
...
describe('Japanese investment column 004 — subsidiary versus branch', () => {
  it('publishes the contracted frontmatter and exactly three FAQs', () => {
    ...
    expect(parsed.data.faq).toEqual([
      ...
      {
        q: '台湾で上場したり投資税額控除を受けたりするには、子会社と支店のどちらを選ぶべきですか？',
        a: listingAnswer,
      },
    ]);
  ...
  it('bounds section 5 to exactly six prose paragraphs covering the incentive window, methods, cap, and eligibility', () => {
    for (const body of [raw, post?.content ?? '']) {
      ...
      expect(section).toContain('100万新台湾ドル（NT$）以上NT$20億以下');
