...
// 4 event cards are laid out from EVENTS_Y+70 at 220px pitch (cardH 200 + gapY 20);
// last card bottom = 70 + 3*220 + 200 = 930 below EVENTS_Y. Reserve enough so the
// appended proof sections don't collide with the event list (was 560 → overlapped).
const EVENTS_H = 970;
...
const eventCards: BuilderCanvasNode[] = events.flatMap((e, i) => {
  ...
  return [
    ...
    heading(`${prefix}-t`, { x: 24, y: 24, width: 502, height: 36 }, e.title, 3, '#1f2622', 'left', prefix),
    createTextNode({
      id: `${prefix}-date`,
      parentId: prefix,
...
const nodes: BuilderCanvasNode[] = assignCanvasNodeZIndices([
  ...
  heading('tpl-cafeevt-events-title', { x: 80, y: EVENTS_Y, width: 400, height: 50 }, '예정된 이벤트', 2, '#1f2622', 'left'),
  ...eventCards,
  createButtonNode({
  ...
  ...buildCategoryProofSections('cafe', 'cafe-events', STAGE_H),
]);
