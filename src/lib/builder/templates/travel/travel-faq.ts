...
function buildFaqItem(faq: Faq, idx: number): BuilderCanvasNode[] {
  ...
  return [
    createContainerNode({
      ...
      background: idx % 2 === 0 ? '#ffffff' : '#f3e6cf',
      borderRadius: 10,
      borderColor: '#e0d3b8',
      borderWidth: 1,
      padding: 20,
    }),
    heading(`${cid}-q`, { x: 20, y: 16, width: CONTENT_W - 60, height: 32 }, faq.q, 3, '#101827', 'left', cid),
    createTextNode({
      id: `${cid}-a`,
      parentId: cid,
...
const nodes: BuilderCanvasNode[] = assignCanvasNodeZIndices([
  ...
  ...buildCategoryProofSections('travel', 'travel-faq', STAGE_H),
]);
