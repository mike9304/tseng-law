...
- **7/21 오후 색인 3사 완료**: GSC 색인 진단(색인14/미색인104 — 89건 발견됨-크롤안됨 → 2026-07-22 정정: 품질+내부링크 병목(권위 단독 설명 폐기)), 우선 5URL 색인요청 성공. Bing 웹마스터 GSC가져오기 등록+사이트맵(Processing). 네이버 소유확인+사이트맵+수집요청 5건. IndexNow 108 URL 핑(HTTP 200). WO-A/WO-B/네이버파일 푸시·배포·라이브검증 그린(새 타이틀 3종·실lastmod·Threads sameAs).
...
## 🆕 2026-07-22 딥리서치 반영 (Claude 104에이전트 · 3표 적대검증)

> 보존본: Obsidian `20-Projects/tseng-law/SEO-딥리서치-2026-07-22.md`
> 확정 14 / 기각 11. 아래만 캠페인에 반영. 기각 주장은 지시·워크오더에 넣지 말 것.

### 진단 정정
- GSC 「발견됨-크롤안됨」 89건 병목 = **콘텐츠 품질 + 내부링크** (1만 페이지 미만에서 크롤예산 이슈 아님). 기존 「권위 부족」 단독 설명은 폐기.
- 품질 재평가는 수개월 단위(Mueller). 4주차 색인률 미개선만으로 전술 실패 단정 금지.

### 전술 조정 (필수)
1. **STEP 15 AI 0건 처방 교체**: `llms.txt 내용 보강` → **인용원(STEP 13) + 페이지 품질/내부링크**. llms.txt 추가 투자 금지(이미 구현 유지는 무해, 무용 확정).
2. **신설 WO-SEO-IL (내부링크)**: 미색인 핵심 한국어 페이지(서비스·가이드·랜딩·검수 통과 칼럼)로 향하는 **문맥형 내부링크** 보강. 홈 허브 링크만 늘리는 방식 금지.
3. **STEP 13 아웃리치 링크 타깃**: 홈이 아니라 **미색인 핵심 서비스/가이드 URL**을 1순위로 (백링크는 크롤 촉발, 색인은 품질 게이트 별도).
4. **네이버**: 단일 주제(대만 법률) 블로그 채널 전제. 시리즈는 의도별(절차·비용·비자·세무) 분리. AI 브리핑 인용은 네이버 생태계 비중 큼(표본 70~83%).
5. **표기 감사**: 사이트·스키마·바이오에서 `변호사` 단독 → **`대만 변호사` 병기** 통일 여부 점검(변협·외국법자문사 기준 + 타깃 키워드 일치).
6. **금지**: 대가성 바이럴/체험단/원고제공형 제3자 게시(2025-02 변협 개정 = 광고). 정보성 자체 채널만.

### 현재 게이트 (2026-07-22 handoff 재확인)
- WO-3a C1~C4: Studio 커밋 `3d535603`, **origin 미푸시(의도적·검수 전)**. 라이브 404. 마커 합 83건. 검수시트 `docs/seo/review-sheet-2026-07-21.txt`.
- STEP 9(변호사 검수) 미완료 → C5~C12(STEP 10·11) 발주 금지.
- 딥리서치 반영 문서화: 본 섹션 + `docs/seo/WO-INTERNAL-LINKS-2026-07-22.md`.
...
| 1 | Google Search Console + Bing 등록 | 에이전트(크롬자동화) | ✅ GSC 기등록 확인(사이트맵 3/10 제출·7/21 정상 크롤)+우선 5URL 색인요청 / Bing 신규등록+사이트맵 제출 | 07-21 |
| 2 | 네이버 서치어드바이저 등록 | 에이전트+사용자(캡차) | ✅ 소유확인(HTML파일)+사이트맵 제출+수집요청 5건 | 07-21 |
...
| 8.5 | WO-SEO-IL: 미색인 핵심 페이지 내부링크 보강 | Codex+검수 | ✅ 구현 완료(미푸시, 07-22) | 07-22 |
| 9 | 변호사 검수 사이클 (3a분) | **사용자(변호사)** | ☐ | |
| 10 | WO-SEO-3b: 칼럼 4편 (C5~C8) + 검수 | Codex+사용자 | ☐ | |
| 11 | WO-SEO-3c: 칼럼 4편 (C9~C12) + 검수 | Codex+사용자 | ☐ | |
| 12 | 배포 후 라이브 일괄 검증 + 색인 요청 | 에이전트 | ◐ WO-A/B 배포·라이브검증 그린, GSC+네이버 색인요청 완료. 칼럼 4편은 검수 후 반복 | 07-21 |
...
- AI 스팟체크 0이면 → **llms.txt 보강 금지**(2026-07-22 확정 무용). 인용원(STEP 13)·내부링크(WO-SEO-IL)·페이지 품질 보강에 집중
