...
const cards: BuilderCanvasNode[] = articles.flatMap((a, i) => {
  ...
  return [
    ...
    heading(`${prefix}-t`, { x: 24, y: 52, width: 312, height: 72 }, a.title, 3, '#1f2a2e', 'left', prefix),
    createTextNode({
      id: `${prefix}-d`,
      parentId: prefix,
      rect: { x: 24, y: 128, width: 312, height: 80 },
      text: a.desc,
      fontSize: 14,
      color: '#1f2a2e',
      lineHeight: 1.5,
...
const nodes: BuilderCanvasNode[] = assignCanvasNodeZIndices([
  ...
  heading('tpl-consblog-grid-title', { x: 80, y: GRID_Y, width: 400, height: 50 }, '최신 글', 2, '#1f2a2e', 'left'),
  ...cards,
  ...buildConsultingProofSections('consulting-blog', STAGE_H),
]);
