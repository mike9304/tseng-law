...
// 3 job cards (pitch 160) + the apply button extend to 70+3*160+48 = 598px below
// POSITIONS_Y; reserve enough so the CULTURE section doesn't overlap them (was 400 → overlap).
const POSITIONS_H = 620;
...
const posCards: BuilderCanvasNode[] = positions.flatMap((p, i) => {
  ...
  return [
    ...
    heading(`${prefix}-t`, { x: 24, y: 24, width: 502, height: 36 }, p.title, 3, '#1f2622', 'left', prefix),
    createTextNode({
      id: `${prefix}-d`,
      parentId: prefix,
...
const nodes: BuilderCanvasNode[] = assignCanvasNodeZIndices([
  ...
  heading('tpl-cafecar-pos-title', { x: 80, y: POSITIONS_Y, width: 400, height: 50 }, '채용 포지션', 2, '#1f2622', 'left'),
  ...posCards,
  createButtonNode({
    ...
    href: '#',
    variant: 'primary',
    style: { backgroundColor: '#c26f3d', borderRadius: 6 },
  }),
  ...
  heading('tpl-cafecar-culture-title', { x: 80, y: 40, width: 400, height: 50 }, '팀 문화', 2, '#1f2622', 'left', 'tpl-cafecar-culture'),
  createTextNode({
  ...
  ...buildCategoryContactMethods('cafe', 'cafe-careers-dir', STAGE_H),
  ...buildCategoryProofSections('cafe', 'cafe-careers', STAGE_H + CONTACT_METHODS_HEIGHT),
]);
