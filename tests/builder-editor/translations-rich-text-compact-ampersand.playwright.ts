...
test.describe('/api/builder/translations/edit compact ampersand rich text persistence', () => {
  ...
  test('preserves compact translated ampersand separators outside adjacent rich text marks', async ({ request }) => {
    const token = `translation-rich-compact-ampersand-${Date.now().toString(36)}`;
    const nodeId = `translation-rich-compact-ampersand-node-${token}`;
