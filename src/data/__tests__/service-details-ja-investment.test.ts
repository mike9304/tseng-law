...
const expectedInvestment = {
  title: '台湾投資・会社設立',
  subtitle: '日本企業をはじめ外国企業の台湾進出を、組織形態の選定から設立・許認可まで一貫支援',
  intro:
    '昊鼎国際法律事務所は、日本企業をはじめ外国企業の台湾進出に際し、台湾子会社・台湾支店・代表者事務所等の組織形態の選定、必要に応じた経済部投資審議司への投資申請、投資資金の送金・投資額審定、会社・支店等の登記、銀行口座開設、営業場所の適法性確認および業種別許認可を支援します。',
  keyPoints: [
    ...
    '外国投資事業の外国籍主管に関する就業許可では、華僑または外国人が保有する当該事業の株式または出資額の合計が、その株式総数または資本総額の3分の1を超える会社の経理人、外国会社の台湾支店の経理人、代表者事務所の代表者等が対象となります。このうち、会社または支店の雇用主が設立1年未満の場合は、実収資本額または台湾における運転資金50万新台湾ドル（NT$）以上、売上高NT$300万以上、輸出入実績50万米ドル以上、または代理手数料20万米ドル以上のいずれかが原則です。設立1年以上の場合は、台湾における直近1年または直近3年平均について、売上高NT$300万以上、輸出入実績50万米ドル以上、または代理手数料20万米ドル以上のいずれかが原則です。代表者事務所は、設立1年以上であれば台湾における業務実績が必要です（設立1年未満は免除）。国内経済の発展に実質的な貢献がある場合または事情が特殊な場合には、特別認定の余地もあります。',
    '投資許可後は、許可内容に従って国外から台湾へ投資資金を送金し、台湾側で投資額の審定を受けます。送金元の国・地域における手続や、利用する送金銀行の最新取扱いは、送金前に個別に確認してください。韓国からの送金である場合に限り、韓国における海外直接投資申告、送金名義、本人来店の要否、インターネットバンキングまたは代理手続の可否を、韓国法および利用する送金銀行の最新取扱いに従って確認します。',
    ...
    '工場登記を免除された製造場所で生産される固形手作り石けんを除き、一定規模の化粧品製造・輸入業者は、対象製品の供給・販売等の前に製品登録を完了し、PIF（化粧品製品情報ファイル）を作成・保存しなければなりません。PIF自体を登録する制度ではありません。虚偽・誇大な表示・宣伝・広告にはNT$4万～NT$20万、医療効能を標榜する場合にはNT$60万～NT$500万の過料が科され得ます。',
    '最低資本額NT$2,500万および全新貨車20両以上という基準は、一般的な物流業全体ではなく、「自動車貨物運送業」を新規に籌設する場合の原則的な基準です。規定に適合する車両、停車場その他の設備も必要です。引越専業、金門・連江地域、個人小貨車貨運業等には別基準または例外があります。既存事業者の買収には投資・会社・運輸関係の許認可確認が必要であり、免許を有する事業者への配送委託は、自社が自動車貨物運送業を営むこととは区別されます。',
...
const cosmeticsAdOverreachPattern =
  /(?:広告(?:の)?違反|虚偽・?誇大)(?:(?!医療効能)[^。]){0,100}(?:500万新台湾ドル|NT\$500万)/;
const logisticsOverreachPattern =
  /(?:物流業(?:(?!ではなく)[^。]){0,120}(?:2,?500万新台湾ドル|NT\$2,?500万|20(?:両|台))|(?:2,?500万新台湾ドル|NT\$2,?500万|20(?:両|台))(?:(?!ではなく)[^。]){0,120}物流業(?!(?:全体)?ではなく))/;
...
const prohibitedProbeCases: Array<{ pattern: RegExp; copy: string }> = [
  ...
  {
    pattern: cosmeticsAdOverreachPattern,
    copy: '化粧品広告の違反には一律NT$500万の過料が科されます。',
  },
  {
  ...
  {
    pattern: logisticsOverreachPattern,
    copy: '最低資本額NT$2,500万と貨車20台が物流業の要件です。',
  },
  {
...
describe('Japanese investment service-detail content', () => {
  ...
  it('qualifies remittance by origin country and limits Korean law to Korean remittance', () => {
    const remittance = getJapaneseServiceDetail('investment')?.keyPoints[3] ?? '';

    expect(remittance.startsWith('投資許可後は、許可内容に従って国外から台湾へ投資資金を送金し')).toBe(true);
    expect(remittance).toContain('送金元の国・地域');
    expect(remittance).toContain('韓国からの送金である場合に限り');
    expect(remittance).toContain('韓国法および利用する送金銀行');
    expect(remittance).not.toMatch(/日本法/);
  });
});
