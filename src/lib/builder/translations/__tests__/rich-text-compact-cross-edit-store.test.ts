...
describe('applyTranslationToLocaleDraft compact cross rich text mark boundaries', () => {
  ...
  it('keeps compact cross separators outside adjacent link and underline marks', async () => {
    ...
    if (node?.kind !== 'text') throw new Error('translated compact cross text node was not written');
