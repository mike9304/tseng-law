...
const tierCards: BuilderCanvasNode[] = tiers.flatMap((t, i) => {
  ...
  const bg = i === 1 ? '#1f2622' : '#efe1c8';
  const textColor = i === 1 ? '#ffffff' : '#1f2622';
  const titleColor = i === 1 ? '#ffffff' : '#1f2622';
  return [
    createContainerNode({
      id: prefix,
...
const nodes: BuilderCanvasNode[] = assignCanvasNodeZIndices([
  ...
  heading('tpl-cafecater-tiers-title', { x: 80, y: TIERS_Y, width: 400, height: 50 }, '케이터링 패키지', 2, '#1f2622', 'left'),
  ...tierCards,
  ...
  ...buildCategoryProofSections('cafe', 'cafe-catering', STAGE_H),
]);
