...
function buildEventCard(event: Event, idx: number): BuilderCanvasNode[] {
  ...
  return [
    ...
    heading(`${cid}-title`, { x: 24, y: 20, width: 400, height: 36 }, event.title, 3, '#2a2418', 'left', cid),
    createTextNode({
      id: `${cid}-date`,
      parentId: cid,
...
const nodes: BuilderCanvasNode[] = assignCanvasNodeZIndices([
  heading('tpl-eduevents-title', { x: MARGIN, y: 50, width: 500, height: 56 }, '행사 일정', 1, '#2a2418'),
  createTextNode({
  ...
  ...buildCategoryProofSections('education', 'education-events', STAGE_H),
]);
