...
function archiveEntry(n: number): BuilderCanvasNode[] {
  ...
  return [
    createContainerNode({
      ...
      background: n % 2 === 1 ? '#f3f1ec' : '#ffffff',
      borderRadius: 8,
      padding: 0,
    }),
    createImageNode({
    ...
    heading(`${cId}-title`, { x: 180, y: 16, width: 500, height: 30 }, a.title, 3, '#1a1a1a', 'left', cId),
    createTextNode({
      id: `${cId}-meta`,
      parentId: cId,
...
const nodes: BuilderCanvasNode[] = assignCanvasNodeZIndices([
  ...
  ...archiveEntry(7),
  ...archiveEntry(8),
  ...buildCategoryProofSections('blog', 'blog-archive', STAGE_H),
]);
