...
export const GUIDANCE_LLMS_NOTICES: Record<GuidanceLocale4, GuidanceLlmsNotices> = {
  vi: {
    consultationNotice:
      'Phần hướng dẫn này được viết bằng tiếng Việt, nhưng việc tư vấn với luật sư chỉ được thực hiện bằng tiếng Anh, tiếng Trung (中文), tiếng Nhật và tiếng Hàn.',
    discoveryNotice:
      'Tệp llms.txt này chỉ là mục lục để tìm các trang công khai; nó không hứa hẹn thứ hạng trên công cụ tìm kiếm, không phải lời giới thiệu hay khuyến nghị của trí tuệ nhân tạo, và cũng không bảo đảm mức độ hiển thị.',
    confidentialNotice:
  ...
  id: {
    ...
    discoveryNotice:
      'Berkas llms.txt ini hanya peta penelusuran halaman publik; berkas ini tidak menjanjikan peringkat pencarian, dukungan resmi, rekomendasi AI, atau jaminan untuk ditampilkan.',
    confidentialNotice:
  ...
  th: {
    ...
    discoveryNotice:
      'ไฟล์ llms.txt นี้เป็นเพียงแผนผังสำหรับค้นหน้าเว็บสาธารณะ ไม่ได้ให้คำมั่นเรื่องอันดับในผลการค้นหา การรับรอง การแนะนำโดย AI หรือการมองเห็นที่รับประกันไว้',
    confidentialNotice:
  ...
  ar: {
    ...
    discoveryNotice:
      'ملف llms.txt هذا مجرد خريطة للوصول إلى الصفحات العامة؛ وهو لا يَعِد بترتيب في نتائج البحث، ولا بتزكية، ولا بتوصية من أنظمة الذكاء الاصطناعي، ولا بقدر مضمون من الظهور.',
  ...
  es: {
    ...
    discoveryNotice:
      'El archivo llms.txt es solo un mapa para localizar páginas públicas; no promete posicionamiento en buscadores, respaldo, recomendación —incluida la de una inteligencia artificial— ni visibilidad garantizada.',
    confidentialNotice:
  ...
  fr: {
    ...
    discoveryNotice:
      'Le fichier llms.txt n’est qu’une carte pour trouver les pages publiques ; il ne promet ni un classement dans les moteurs de recherche, ni une approbation, ni une recommandation par une intelligence artificielle, ni une visibilité particulière.',
    confidentialNotice:
  ...
  ru: {
    ...
    discoveryNotice:
      'Файл llms.txt — лишь карта для поиска открытых страниц; он не обещает места в поиске, одобрения, рекомендации искусственного интеллекта или гарантированного показа.',
    confidentialNotice:
  ...
  tr: {
    ...
    discoveryNotice:
      'llms.txt dosyası yalnızca kamuya açık sayfaları bulmak için bir haritadır; arama sıralaması, tavsiye, yapay zekâ önerisi veya görünürlük güvencesi vaat etmez.',
    confidentialNotice:
  ...
  it: {
    ...
    discoveryNotice:
      'Il file llms.txt è soltanto una mappa per trovare le pagine pubbliche; non promette posizionamento nei motori di ricerca, avallo, raccomandazione di un’intelligenza artificiale o visibilità garantita.',
    confidentialNotice:
  ...
  hi: {
    ...
    discoveryNotice:
      'llms.txt फ़ाइल केवल सार्वजनिक पृष्ठ खोजने का मानचित्र है; यह खोज में स्थान, समर्थन, कृत्रिम बुद्धि की अनुशंसा या सुनिश्चित दृश्यता का वादा नहीं करती।',
    confidentialNotice:
  ...
  da: {
    ...
    discoveryNotice:
      'Filen llms.txt er kun et kort til at finde offentlige sider; den lover ikke søgeplacering, omtale, anbefaling fra kunstig intelligens eller garanteret synlighed.',
    confidentialNotice:
  ...
  ro: {
    ...
    discoveryNotice:
      'Fișierul llms.txt este doar o hartă pentru găsirea paginilor publice; nu promite poziționare în căutare, o recomandare, o recomandare din partea inteligenței artificiale și nici vizibilitate garantată.',
    confidentialNotice:
  ...
  uk: {
    ...
    discoveryNotice:
      'Файл llms.txt — це лише мапа для пошуку публічних сторінок; він не обіцяє місця в пошуку, схвалення, рекомендації штучного інтелекту чи гарантованої видимості.',
    confidentialNotice:
  ...
  el: {
    ...
    discoveryNotice:
      'Το αρχείο llms.txt χρησιμεύει μόνο ως χάρτης για την εύρεση των δημόσιων σελίδων· δεν υπόσχεται ούτε θέση στην αναζήτηση, ούτε έγκριση, ούτε σύσταση από τεχνητή νοημοσύνη, ούτε εγγυημένη προβολή.',
    confidentialNotice:
  ...
  he: {
    ...
    discoveryNotice:
      'הקובץ llms.txt הוא מפה בלבד לאיתור העמודים הפומביים; אין בו הבטחה לדירוג בחיפוש, לאישור מטעם המשרד, להמלצה של בינה מלאכותית או לחשיפה מובטחת.',
    confidentialNotice:
  ...
  bn: {
    ...
    discoveryNotice:
      'llms.txt ফাইল কেবল সর্বজনীন পাতা খুঁজে পাওয়ার মানচিত্র; এটি অনুসন্ধানে স্থান, প্রচার, কৃত্রিম বুদ্ধিমত্তার সুপারিশ বা নিশ্চিত দৃশ্যমানতার প্রতিশ্রুতি দেয় না।',
    confidentialNotice:
  ...
  ur: {
    ...
    discoveryNotice:
      'llms.txt فائل صرف عوامی صفحات تلاش کرنے کا نقشہ ہے؛ یہ تلاش میں جگہ، سفارش، مصنوعی ذہانت کی سفارش یا ضمانت شدہ نمائش کا وعدہ نہیں کرتی۔',
    confidentialNotice:
  ...
  my: {
    ...
    discoveryNotice:
      'llms.txt ဖိုင်သည် အများပြည်သူ စာမျက်နှာများကို ရှာရန် မြေပုံသာ ဖြစ်သည်။ ရှာဖွေမှုရလဒ်ထဲက အဆင့်၊ ထောက်ခံချက်၊ ဉာဏ်ရည်တု၏ အကြံပြုချက် သို့မဟုတ် မြင်သာမှုကို အာမမခံပါ။',
    confidentialNotice:
  ...
  ne: {
    ...
    discoveryNotice:
      'llms.txt फाइल सार्वजनिक पृष्ठ खोज्ने नक्सा मात्र हो; यो खोजमा स्थान, सिफारिस, कृत्रिम बुद्धिको सिफारिस वा सुनिश्चित दृश्यताको वाचा गर्दैन।',
    confidentialNotice:
  ...
  mn: {
    ...
    discoveryNotice:
      'llms.txt файл нь зөвхөн нээлттэй хуудсыг олоход зориулсан газрын зураг юм; энэ нь зэрэглэл, зөвлөмж, хиймэл оюуны зөвлөмж, сурталчилгааг амлахгүй.',
    confidentialNotice:
  ...
  bg: {
    ...
    discoveryNotice:
      'Файлът llms.txt е само карта за намиране на публичните страници; той не обещава класиране в търсенето, одобрение, препоръка от изкуствен интелект или гарантирано показване.',
    confidentialNotice:
  ...
  hr: {
    ...
    discoveryNotice:
      'Datoteka llms.txt je samo karta za pronalaženje javnih stranica; ne obećava poredak u pretraživanju, službenu preporuku, preporuku umjetne inteligencije ni zajamčenu vidljivost.',
    confidentialNotice:
  ...
  sr: {
    ...
    discoveryNotice:
      'Datoteka llms.txt samo je mapa za pronalaženje javnih stranica; ne obećava plasman u pretrazi, odobravanje, preporuku veštačke inteligencije niti zagarantovanu vidljivost.',
    confidentialNotice:
  ...
  et: {
    ...
    discoveryNotice:
      'Fail llms.txt on üksnes ülevaade avalike lehtede leidmiseks; see ei luba kohta otsingutulemustes, soovitust, tehisintellekti soovitust ega nähtavust.',
    confidentialNotice:
  ...
  is: {
    ...
    discoveryNotice:
      'Skráin llms.txt er aðeins kort til að finna opinberar síður; hún lofar hvorki sæti í leitarvél, meðmæli, tillögu gervigreindar né tryggða birtingu.',
    confidentialNotice:
