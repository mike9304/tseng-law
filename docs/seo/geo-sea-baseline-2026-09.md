...
목적: 동남아 사용자가 대만 법률 문제를 생성형 AI에 물을 때 `tseng-law.com`이 인용되는지를 **분모부터 고정**한다. 이 문서는 채점표다. 2026-09-14 브라우저 재실측: **Perplexity 31/31 완료(Y9·N22)**. **ChatGPT 31/31 완료(Y1·N30)**. Google AIO는 reCAPTCHA로 중단(§2.6). GM/CL/GR은 아직 대기.
...
### 2.6 2회차 실측 로그 (2026-09-14 · 손빗 브라우저)

**Perplexity(PP):** 사람 브라우저로 S0-01~31 전부 실측. **Y=9 · N=22 · 미실측=0.** 산출물: `exports` 동기 `/workspace/exports/sea-2026-09/aicite/2026-09-14/perplexity-all.md` 및 `PP-S0-*.png`.
인용 Y: S0-03, S0-04, S0-07, S0-08, S0-12, S0-13, S0-20, S0-23, S0-31.

**Google AIO:** S0-01 검색에서 unusual-traffic reCAPTCHA → 배치 중단. 전 문항 `AIO=미실측(SERP CAPTCHA)`.

**ChatGPT(CG):** 브라우저 31/31 완료. **Y=1(S0-17)·N=30·미실측=0.** 산출물 chatgpt-all.md.

**Gemini(GM):** S0-01~08 시도 — Google 계정 로그아웃·로그인 벽 → 전 문항 `GM=미실측(로그인 필요)` (배치 중단).

**CL / GR:** 오늘 미시도 → `미실측(미시도·오늘)`.
