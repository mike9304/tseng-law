...
const posCards: BuilderCanvasNode[] = positions.flatMap((p, i) => {
  ...
  return [
    ...
    heading(`${prefix}-t`, { x: 24, y: 24, width: 312, height: 36 }, p.title, 3, '#16191f', 'left', prefix),
    createTextNode({
      id: `${prefix}-d`,
      parentId: prefix,
...
const nodes: BuilderCanvasNode[] = assignCanvasNodeZIndices([
  ...
  heading('tpl-stupcar-pos-title', { x: 80, y: POSITIONS_Y, width: 400, height: 50 }, '오픈 포지션', 2, '#16191f', 'left'),
  ...posCards,
  createButtonNode({
  ...
  heading('tpl-stupcar-perks-title', { x: 80, y: 40, width: 400, height: 50 }, '복지 & 문화', 2, '#16191f', 'left', 'tpl-stupcar-perks'),
  createTextNode({
  ...
  ...buildCategoryProofSections('startup', 'startup-careers', STAGE_H),
]);
