...
import { buildConsultingContactMethods, buildConsultingProofSections, CONTACT_METHODS_HEIGHT, SUBPAGE_PROOF_HEIGHT } from '../_shared/subpage-proof-sections';
...
const nodes: BuilderCanvasNode[] = assignCanvasNodeZIndices([
  ...
  heading('tpl-conscon-form-title', { x: 80, y: FORM_Y, width: 400, height: 50 }, '무료 상담 신청', 2, '#1f2a2e', 'left'),
  createContainerNode({
    id: 'tpl-conscon-form',
  ...
  createButtonNode({
    ...
    href: '#',
    variant: 'primary',
    style: { backgroundColor: '#3f5f5b', borderRadius: 6 },
  }),
  ...
  /* ── Office locations ───────────────────────────────────── */
  heading('tpl-conscon-office-title', { x: 80, y: OFFICES_Y, width: 400, height: 50 }, '오피스 위치', 2, '#1f2a2e', 'left'),
  createContainerNode({
  ...
  heading('tpl-conscon-office-1-t', { x: 24, y: 24, width: 312, height: 36 }, '서울 본사', 3, '#1f2a2e', 'left', 'tpl-conscon-office-1'),
  createTextNode({
  ...
  heading('tpl-conscon-office-2-t', { x: 24, y: 24, width: 312, height: 36 }, '부산 지사', 3, '#1f2a2e', 'left', 'tpl-conscon-office-2'),
  createTextNode({
  ...
  ...buildConsultingContactMethods('consulting-contact-dir', STAGE_H),
  ...buildConsultingProofSections('consulting-contact', STAGE_H + CONTACT_METHODS_HEIGHT),
]);
