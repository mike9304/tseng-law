...
test.describe('/api/builder/translations/edit compact semicolon rich text persistence', () => {
  ...
  test('preserves compact translated semicolon separators outside adjacent rich text marks', async ({ request }) => {
    const token = `translation-rich-compact-semicolon-${Date.now().toString(36)}`;
    const nodeId = `translation-rich-compact-semicolon-node-${token}`;
