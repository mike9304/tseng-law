...
const cards: BuilderCanvasNode[] = members.flatMap((m, i) => {
  ...
  return [
    ...
    heading(`${prefix}-name`, { x: 24, y: 24, width: 312, height: 36 }, m.name, 3, '#1f2a2e', 'left', prefix),
    createTextNode({
      id: `${prefix}-exp`,
      parentId: prefix,
...
const nodes: BuilderCanvasNode[] = assignCanvasNodeZIndices([
  ...
  heading('tpl-consteam-grid-title', { x: 80, y: GRID_Y, width: 400, height: 50 }, '컨설턴트 소개', 2, '#1f2a2e', 'left'),
  ...cards,
  ...buildConsultingProofSections('consulting-team', STAGE_H),
]);
