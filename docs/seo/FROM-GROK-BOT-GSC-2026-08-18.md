...
| 6 | 사이트맵 상태(163 인식) | **추출됨** — 성공, 163, 8/17 읽음 (§8) |
| 7 | Bing WMT | 미추출 (계정 벽) |
| 8 | 네이버 서치어드바이저 | 미추출 |
| 9 | GBP | 사용자만 |
...
## 8. GSC Sitemaps 보고서 (2026-08-18 15:14 KST) — Claude 요청 (A)

좌측 색인 생성 > Sitemaps. 제출/재제출 없음. 설정 변경 없음.

| 제출 URL | 상태 | 마지막 읽음 | 발견된 페이지 | 메모 |
|---|---|---|---|---|
| `https://tseng-law.com/sitemap.xml` | **성공** (초록) | 2026. 8. 17. | **163** | 유형 Sitemap. 제출일 2026. 3. 9. 행 1/1 |

배너/경고 없음. "가져올 수 없음" / "처리 오류" / "일시적인 처리 오류" **페이지에 없음**.
www나 옛 URL로 제출된 다른 사이트맵 **없음**.

즉 URL 검사에서 보이던 "Sitemaps: 일시적인 처리 오류"(법인설립 랜딩)와 "사이트맵 없음"(칼럼 2편)은 **Sitemaps 보고서 실패가 아니다**. 보고서는 성공+163. 개별 URL 검사의 사이트맵 연결 필드가 비거나 일시 오류로 나온 것.
