...
function buildServiceCard(svc: Service, idx: number): BuilderCanvasNode[] {
  ...
  return [
    ...
    heading(`${cid}-title`, { x: 24, y: 88, width: 300, height: 36 }, svc.title, 3, '#0f0f12', 'left', cid),
    createTextNode({
      id: `${cid}-desc`,
      parentId: cid,
...
const nodes: BuilderCanvasNode[] = assignCanvasNodeZIndices([
  heading('tpl-creativesvc-title', { x: MARGIN, y: 50, width: 500, height: 56 }, '서비스', 1, '#0f0f12'),
  createTextNode({
  ...
  ...buildCategoryProofSections('creative', 'creative-services', STAGE_H),
]);
