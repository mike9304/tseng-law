...
const TEAM_H = 370; // tightened from 520: section over-reserved, left a large empty gap
...
const vetCards: BuilderCanvasNode[] = vets.flatMap((v, i) => {
  ...
  return [
    ...
    heading(`${prefix}-name`, { x: 24, y: 24, width: 212, height: 36 }, v.name, 3, '#2b2018', 'left', prefix),
    createTextNode({
      id: `${prefix}-spec`,
      parentId: prefix,
...
const nodes: BuilderCanvasNode[] = assignCanvasNodeZIndices([
  ...
  heading('tpl-petteam-grid-title', { x: 80, y: TEAM_Y, width: 400, height: 50 }, '수의사 프로필', 2, '#2b2018', 'left'),
  ...vetCards,
  ...buildCategoryProofSections('pet', 'pet-team', STAGE_H),
]);
