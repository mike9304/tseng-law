...
const ISSUES_H = 352; // +32 for 2-line titles
...
function issueCard(n: number): BuilderCanvasNode[] {
  ...
  return [
    createContainerNode({
      id: cId,
      rect: { x, y: ISSUES_Y + 70, width: 360, height: 252 },
      background: '#f3f1ec',
      borderRadius: 12,
      padding: 24,
    }),
    heading(`${cId}-title`, { x: 24, y: 16, width: 312, height: 72 }, issue.title, 3, '#1a1a1a', 'left', cId),
    createTextNode({
      id: `${cId}-date`,
      parentId: cId,
      rect: { x: 24, y: 92, width: 200, height: 24 },
      text: issue.date,
      fontSize: 13,
      color: '#66645e',
    }),
    createTextNode({
      id: `${cId}-preview`,
      parentId: cId,
      rect: { x: 24, y: 128, width: 312, height: 60 },
    ...
    createButtonNode({
      id: `${cId}-btn`,
      parentId: cId,
      rect: { x: 24, y: 200, width: 120, height: 36 },
      label: '읽어보기',
      href: '#',
      variant: 'outline',
...
const nodes: BuilderCanvasNode[] = assignCanvasNodeZIndices([
  ...
  createButtonNode({
    ...
    href: '#',
    variant: 'primary',
    style: { backgroundColor: '#9a6a3a', borderRadius: 6 },
  }),
  ...
  ...issueCard(2),
  ...issueCard(3),
  ...buildCategoryProofSections('blog', 'blog-newsletter', STAGE_H),
]);
