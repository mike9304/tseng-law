...
function buildStepCard(step: ProcessStep, idx: number): BuilderCanvasNode[] {
  ...
  return [
    createContainerNode({
      ...
      background: isOdd ? '#0f0f12' : '#f3f3f4',
      borderRadius: 12,
      padding: 32,
    }),
    createTextNode({
    ...
    createTextNode({
      ...
      text: step.desc,
      fontSize: 15,
      color: isOdd ? 'rgba(255,255,255,0.85)' : '#0f0f12',
      lineHeight: 1.6,
    }),
  ];
}
...
const nodes: BuilderCanvasNode[] = assignCanvasNodeZIndices([
  heading('tpl-creativeprocess-title', { x: MARGIN, y: 50, width: 500, height: 56 }, '작업 프로세스', 1, '#0f0f12'),
  createTextNode({
  ...
  ...buildCategoryProofSections('creative', 'creative-process', STAGE_H),
]);
