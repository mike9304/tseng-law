...
test.describe('/api/builder/translations/edit compact plus rich text persistence', () => {
  ...
  test('preserves compact translated plus separators outside adjacent rich text marks', async ({ request }) => {
    const token = `translation-rich-compact-plus-${Date.now().toString(36)}`;
    const nodeId = `translation-rich-compact-plus-node-${token}`;
