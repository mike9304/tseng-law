...
function buildBillingFaqItem(faq: BillingFaq, idx: number): BuilderCanvasNode[] {
  ...
  return [
    createContainerNode({
      ...
      background: idx % 2 === 0 ? '#ffffff' : '#dff0ea',
      borderRadius: 10,
      borderColor: '#c5ded7',
      borderWidth: 1,
      padding: 20,
    }),
    heading(`${cid}-q`, { x: 20, y: 12, width: CONTENT_W - 60, height: 28 }, faq.q, 3, '#123c32', 'left', cid),
    createTextNode({
      id: `${cid}-a`,
      parentId: cid,
...
const nodes: BuilderCanvasNode[] = assignCanvasNodeZIndices([
  ...
  ...buildCategoryProofSections('health', 'health-insurance', STAGE_H),
]);
