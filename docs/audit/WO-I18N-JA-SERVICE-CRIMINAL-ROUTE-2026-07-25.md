# WO-I18N-JA-SERVICE-CRIMINAL-ROUTE — Publish reviewed Japanese criminal detail
...
Publish the approved Japanese criminal-service record at
`/ja/services/criminal`, preserving the existing Japanese investment, civil,
family and labor routes and every KO, ZH-Hant and EN service-detail behavior.

Only `investment`, `civil`, `family`, `labor` and `criminal` may be published
in Japanese after this unit. IP remains unavailable until its own content and
route reviews pass.
...
3. `src/app/[locale]/services/[slug]/__tests__/ja-criminal-page.test.tsx` (new)
...
## Japanese criminal page

`/ja/services/criminal` must:

- render the exact committed criminal title, subtitle, intro and five ordered
  points;
...
- render the exact Japanese empty-column message because the reviewed base
  `criminal.columnSlugs` list is currently empty;
- not invent or attach related columns in this unit;
- emit Japanese BreadcrumbList, route LegalService and Person JSON-LD;
- produce canonical `/ja/services/criminal`, `content-language=ja`, Open
  Graph locale `ja_JP`, and exact KO/ZH-Hant/EN/JA plus Korean x-default
  alternates;
- never render the stale KO/ZH-Hant/EN criminal body through the Japanese
  route;
- use `toBuilderLocale('ja') === 'en'` only for dynamic-template visibility.

Normalized input such as `CRIMINAL` must permanently redirect to
`/ja/services/criminal`.

`ip` must continue to return empty metadata and 404 without builder
service-source or template-visibility reads.

Existing Japanese investment, civil, family and labor behavior remains
unchanged. Builder-only locale types must not be widened for Japanese.

## Language-switch contract

`jaLanguageSwitchTarget()` must preserve all five approved detail paths:
...
IP continues to fall back to `/ja/services`.

On `/ja/services/criminal`, the public flag switcher must resolve:

```text
🇰🇷 KR -> /ko/services/criminal
🇯🇵 JP -> /ja/services/criminal
🇹🇼 TW -> /zh-hant/services/criminal
🇺🇸 EN -> /en/services/criminal
```

## Sitemap contract

Add `/ja/services/criminal` once with priority `0.72`, all four locale
alternates and Korean x-default. Preserve the four earlier Japanese service
details once each.

Tests must prove:

- investment, civil, family, labor and criminal each occur exactly once with
  exact alternate URLs;
- Japanese IP detail is absent;
- the count fixture changes only by the new criminal URL:
  `beforeFiltering: 155`, `afterFiltering: 146`, `removed: 9`.

## Focused test requirements

### Existing investment route suite

- remove `criminal` from `unsupportedJapaneseSlugs`;
- add the Japanese criminal static param after labor;
- retain every existing investment and unsupported-slug assertion otherwise.

### New criminal route suite

Prove:

1. exact Japanese metadata, canonical, content-language, `ja_JP` and all five
   alternate entries;
2. exact approved criminal body and five ordered points render;
3. `getServiceArea('criminal').columnSlugs` is exactly empty, the Japanese
...
5. BreadcrumbList, route LegalService and Person JSON-LD render with criminal
   and attorney URLs;
6. stale KO and ZH-Hant criminal body, EN base intro and English UI fallback
   are absent;
7. the builder service source is never read for Japanese criminal;
8. template visibility is read with builder locale `en`;
9. uppercase `CRIMINAL` redirects to the lowercase Japanese path.

## Forbidden scope

- Editing `src/data/service-details-ja.ts`, data tests or base
  `src/data/service-details.ts`
- Publishing Japanese IP
...
- `/ja/services/criminal` returns 200, has `<html lang="ja">` and the exact
  canonical;
...
- the Japanese empty-column message renders and there are no related-column
  links;
- attorney/contact links and all four exact flag hrefs preserve the
  `criminal` path;
...
- `/ja/services/ip` remains 404;
- sitemap includes criminal exactly once and excludes IP.
