...
function buildStepCard(step: Step, idx: number): BuilderCanvasNode[] {
  ...
  return [
    createContainerNode({
      ...
      background: idx % 2 === 0 ? '#ece7df' : '#ffffff',
      borderRadius: 10,
      borderColor: '#d6cbbd',
      borderWidth: 1,
      padding: 24,
    ...
    heading(`${cid}-title`, { x: 100, y: 20, width: 300, height: 32 }, step.title, 3, '#26313d', 'left', cid),
    createTextNode({
      id: `${cid}-desc`,
      parentId: cid,
...
const nodes: BuilderCanvasNode[] = assignCanvasNodeZIndices([
  ...
  ...buildCategoryProofSections('realestate', 'realestate-buying', STAGE_H),
]);
