...
// 반복(Loop) 섹션 — pulse 프리셋 + duration/intensity 파라미터.
...
export const W170_loopPulse: CheckpointDefinition = {
  id: 'W170',
  title: '반복(Loop) 애니메이션 - pulse: 선택 → 적용(duration/intensity 활성화 + 값 설정) → undo',
  verification:
    '노드 선택 → 애니메이션 탭 → 반복 프리셋을 pulse 로 변경 → 콤보박스 값 반영 + duration/intensity 활성화(preset=none 게이트 해제) → intensity 값 설정 → undo 복원',
  async run({ page, baseUrl, recordEvidence, log }) {
    ...
    if (!(await tabBtn.isVisible().catch(() => false))) {
      ...
      await recordEvidence('w170-anim-tab-missing');
      return { findings };
    }
    ...
    if ((await combo.count().catch(() => 0)) === 0) {
      ...
      await recordEvidence('w170-loop-combo-missing');
      return { findings };
    }
    ...
    log(`duration/intensity disabled(변경 전, none 게이트): ${durationGateBefore} / ${intensityGateBefore}`);

    log('반복 프리셋 → pulse');
    await combo.selectOption('pulse');
    ...
    await recordEvidence('w170-pulse-applied');
    if (applied !== 'pulse') {
      findings.push({
        severity: 'blocker',
        summary: `반복 프리셋이 pulse 로 commit 되지 않음 (현재="${applied}")`,
      });
    }
    if (durationDisabled) {
      findings.push({
        severity: 'visual',
        summary: 'pulse 적용 후에도 반복 duration 컨트롤이 비활성 상태임 (preset=none 게이트가 풀리지 않음)',
      });
    }
    if (intensityDisabled) {
      findings.push({
        severity: 'visual',
        summary: 'pulse 적용 후에도 반복 intensity 컨트롤이 비활성 상태임',
      });
    }

    // intensity 값 설정(적용) 검증.
    if (!intensityDisabled) {
      const targetIntensity = 60;
      const before = Number(await intensityInput.inputValue().catch(() => '0'));
      log(`intensity ${before} → ${targetIntensity}`);
      ...
      await recordEvidence('w170-pulse-intensity-set');
      if (committed !== targetIntensity) {
        findings.push({
          severity: 'blocker',
          summary: `반복 intensity 값(${targetIntensity})이 commit 되지 않음 (현재="${committed}")`,
        });
      }
    }

    log('undo 로 복원');
    // preset 셋 + intensity 변경 → undo 2회 + 수동 보정.
    for (let i = 0; i < 2; i += 1) {
    ...
    await recordEvidence('w170-undone');

    // Published-runtime emission 검증: pulse + intensity(60) 를 다시 적용해 발행하고
    // 게시 페이지에 data-anim-loop="pulse" + data-anim-loop-intensity="60" 가 노출되는지 확인.
    // 루프 런타임은 @keyframes builder-loop-pulse (globals.css) + AnimationsRoot 가
    // 주입하는 --builder-anim-loop-* CSS var 로 실행됨.
    log('pulse + intensity(60) 재적용 후 발행하여 게시 emission 확인');
    await combo.selectOption('pulse');
    await page.waitForTimeout(200);
    if (await intensityInput.isVisible().catch(() => false)) {
      await intensityInput.fill('60');
      await intensityInput.press('Enter');
      await page.waitForTimeout(200);
    }
    const pulsePublished = await publishSiteViaApi(page, homePageId, log);
    if (!pulsePublished) {
      findings.push({
        severity: 'blocker',
        summary: '발행 플로우를 완료할 수 없어 게시 페이지의 loop(pulse) emission 검증 불가',
      });
    } else {
      const { status, body } = await fetchPublishedBody(page, baseUrl);
      const loopAttr = body.includes('data-anim-loop="pulse"');
      const intensityAttr = body.includes('data-anim-loop-intensity="60"');
      log(`게시 페이지 status=${status}, data-anim-loop="pulse"=${loopAttr}, loop-intensity="60"=${intensityAttr}`);
      await recordEvidence('w170-published-emission');
      if (loopAttr && intensityAttr) {
        log('게시 런타임 확인: @keyframes builder-loop-pulse + AnimationsRoot CSS var 로 pulse 루프 실행 (에디터 캔버스는 루프 미리보기 없음)');
      } else {
        findings.push({
          severity: 'blocker',
          summary: `게시 페이지에 pulse loop emission 이 없음 (data-anim-loop="pulse"=${loopAttr}, loop-intensity="60"=${intensityAttr}) — @keyframes builder-loop-pulse 가 노드에 연결되지 않음`,
        });
      }
    }

    // 초안 복원 + 게시본 정리: 원래값으로 되돌리고 재발행하여 QA 효과가 home 에 남지 않도록 정리.
    log('초안 pulse → 원래값 복원 후 재발행');
    await combo.selectOption(original || 'none').catch(() => undefined);
    await page.waitForTimeout(200);
    await publishSiteViaApi(page, homePageId, log).catch(() => undefined);
    await recordEvidence('w170-published-restored');
