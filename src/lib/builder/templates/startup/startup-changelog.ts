...
const entries: BuilderCanvasNode[] = versions.flatMap((v, i) => {
  ...
  return [
    createContainerNode({
      ...
      background: i % 2 === 0 ? '#eef3fb' : '#ffffff',
      borderRadius: 12,
      padding: 24,
    }),
    heading(`${prefix}-ver`, { x: 24, y: 16, width: 200, height: 32 }, v.ver, 3, '#16191f', 'left', prefix),
    createTextNode({
      id: `${prefix}-date`,
      parentId: prefix,
...
const nodes: BuilderCanvasNode[] = assignCanvasNodeZIndices([
  ...
  heading('tpl-stuplog-timeline-title', { x: 80, y: TIMELINE_Y, width: 400, height: 50 }, '릴리즈 노트', 2, '#16191f', 'left'),
  ...entries,
  ...buildCategoryProofSections('startup', 'startup-changelog', STAGE_H),
]);
