...
const HEADER_H = 140;
const CARD_W = 370;
const CARD_H = 420; // +40 so 2-line Korean titles fit without crowding the date
const GAP = 24;
const COLS = 3;
...
function buildBlogCard(post: BlogPost, idx: number): BuilderCanvasNode[] {
  ...
  return [
    ...
    heading(`${cid}-title`, { x: 20, y: 196, width: 330, height: 72 }, post.title, 3, '#0f0f12', 'left', cid),
    createTextNode({
      id: `${cid}-date`,
      parentId: cid,
      rect: { x: 20, y: 274, width: 120, height: 22 },
      text: post.date,
      fontSize: 13,
      color: '#66666e',
    }),
    createTextNode({
      id: `${cid}-excerpt`,
      parentId: cid,
      rect: { x: 20, y: 304, width: 330, height: 60 },
      text: post.excerpt,
      fontSize: 14,
      color: '#0f0f12',
      lineHeight: 1.5,
    ...
    createButtonNode({
      id: `${cid}-btn`,
      parentId: cid,
      rect: { x: 20, y: 372, width: 110, height: 32 },
      label: '더 보기',
      href: '#',
      variant: 'link',
...
const nodes: BuilderCanvasNode[] = assignCanvasNodeZIndices([
  heading('tpl-creativeblog-title', { x: MARGIN, y: 50, width: 500, height: 56 }, '디자인 인사이트', 1, '#0f0f12'),
  createTextNode({
  ...
  ...buildCategoryProofSections('creative', 'creative-blog', STAGE_H),
]);
