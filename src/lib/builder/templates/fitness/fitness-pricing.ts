...
function buildTierCard(tier: PriceTier, idx: number): BuilderCanvasNode[] {
  ...
  const bg = tier.highlight ? '#14181f' : '#ffffff';
  const titleColor = tier.highlight ? '#ffffff' : '#14181f';
  const textColor = tier.highlight ? 'rgba(255,255,255,0.85)' : '#14181f';
  ...
  return [
    createContainerNode({
      ...
      background: bg,
      borderRadius: 16,
      borderColor: tier.highlight ? '#14181f' : '#dde1e8',
    ...
    createButtonNode({
      ...
      style: { backgroundColor: tier.highlight ? '#e0532b' : 'transparent', borderRadius: 6 },
    }),
  ];
}
...
const nodes: BuilderCanvasNode[] = assignCanvasNodeZIndices([
  ...
  ...buildCategoryProofSections('fitness', 'fitness-pricing', STAGE_H),
]);
