...
- [x] **M5 회귀 게이트** ✅사이클 3(`npm run qa` exit 0: typecheck·lint·1203파일 9462테스트·security / `next build` 성공 / 수용 프로브 전 항목 통과·콘솔 오류 0·수평 오버플로 0) — 배치 5 후 재실행 예정: `npm run qa`(typecheck+lint+test:unit+security) 그린 + `next build` 성공 + 4언어×주요 페이지 로컬 스크린샷에서 수평 오버플로 0·12px 미만 텍스트 0(수용 기준 항목)·콘솔 오류 0.
