...
const tierCards: BuilderCanvasNode[] = tiers.flatMap((t, i) => {
  ...
  const bg = i === 1 ? '#16191f' : '#eef3fb';
  const textColor = i === 1 ? '#ffffff' : '#16191f';
  const titleColor = i === 1 ? '#ffffff' : '#16191f';
  return [
    createContainerNode({
      id: prefix,
...
const nodes: BuilderCanvasNode[] = assignCanvasNodeZIndices([
  ...
  heading('tpl-stupprc-tiers-title', { x: 80, y: TIERS_Y, width: 400, height: 50 }, '플랜 비교', 2, '#16191f', 'left'),
  ...tierCards,
  ...
  heading('tpl-stupprc-compare-title', { x: 80, y: 40, width: 400, height: 50 }, '기능 비교표', 2, '#16191f', 'left', 'tpl-stupprc-compare'),
  createTextNode({
  ...
  ...buildCategoryProofSections('startup', 'startup-pricing', STAGE_H),
]);
