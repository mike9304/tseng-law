...
test.describe('/api/builder/translations/edit compact colon rich text persistence', () => {
  ...
  test('preserves compact translated colon separators outside adjacent rich text marks', async ({ request }) => {
    const token = `translation-rich-compact-colon-${Date.now().toString(36)}`;
    const nodeId = `translation-rich-compact-colon-node-${token}`;
