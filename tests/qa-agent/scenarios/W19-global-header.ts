...
async function headerSignature(page: Page): Promise<string | null> {
  const logo = page.locator('.builder-site-header .logo-kr').first();
  if (!(await logo.isVisible().catch(() => false))) return null;
  const text = (await logo.innerText().catch(() => '')).trim();
  return text || null;
}

export const W19_globalHeader: CheckpointDefinition = {
  id: 'W19',
  title: 'Header 가 모든 페이지 공유 (글로벌 요소)',
  verification: '페이지 전환 후에도 동일한 글로벌 헤더 브랜드가 유지되는지 확인',
  async run({ page, baseUrl, recordEvidence, log }) {
    ...
    log('글로벌 헤더 시그니처 캡처 (페이지 A)');
    const sigA = await headerSignature(page);
    if (!sigA) {
      findings.push({ severity: 'blocker', summary: '글로벌 헤더(.builder-site-header .logo-kr)를 찾을 수 없음' });
      await recordEvidence('header-missing-page-a');
      return { findings };
    }
    await recordEvidence('header-page-a');
    ...
    if (result.pageCount > 0 && result.pageCount < 2) {
      findings.push({
        severity: 'visual',
        summary: '페이지가 1개뿐이라 글로벌 헤더 비교 불가 (fixture 제약)',
      });
      return { findings };
    }
    if (!result.switched) {
    ...
    log('전환 후 글로벌 헤더 시그니처 비교 (페이지 B)');
    const sigB = await headerSignature(page);
    if (!sigB) {
      findings.push({ severity: 'blocker', summary: '페이지 전환 후 글로벌 헤더가 사라짐 (글로벌 아님)' });
      await recordEvidence('header-missing-page-b');
      return { findings };
    }
    if (sigA !== sigB) {
      findings.push({
        severity: 'blocker',
        summary: `페이지마다 헤더 브랜드가 다름 — A="${sigA}" / B="${sigB}"`,
      });
    }
    await recordEvidence('header-page-b');
