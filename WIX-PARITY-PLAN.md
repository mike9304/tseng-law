...
| M159 | CMS | Dataset/repeater/dynamic page binding | F17~F26 | 32h | M158 | 🟡 home insights + services runtime datasets, generic repeater preview contract, scene/layers repeater preview nodes, selected-record dynamic URL + per-item SEO preview, service dataset inspector/E2E green, canvas service/FAQ item click focuses persisted collection editor, admin canvas service/FAQ repeater quick edit, dynamic template editor v0 block/record preview + draft save/load / full visual repeater authoring + dynamic template publish lifecycle remain |
