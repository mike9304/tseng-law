...
import { buildConsultingProofSections, SUBPAGE_PROOF_HEIGHT } from '../_shared/subpage-proof-sections';

const W = 1280;
const HERO_H = 300;
const POSITIONS_Y = HERO_H + 80;
// 4 job cards (pitch 160) + the apply button extend to 70+4*160-20+48 = 738px below
// POSITIONS_Y; reserve enough so the CULTURE section doesn't overlap them (was 400 → overlap).
const POSITIONS_H = 760;
...
const posCards: BuilderCanvasNode[] = positions.flatMap((p, i) => {
  ...
  return [
    ...
    heading(`${prefix}-t`, { x: 24, y: 24, width: 502, height: 36 }, p.title, 3, '#1f2a2e', 'left', prefix),
    createTextNode({
      id: `${prefix}-d`,
      parentId: prefix,
...
const nodes: BuilderCanvasNode[] = assignCanvasNodeZIndices([
  ...
  heading('tpl-conscar-pos-title', { x: 80, y: POSITIONS_Y, width: 400, height: 50 }, '채용 중인 포지션', 2, '#1f2a2e', 'left'),
  ...posCards,
  createButtonNode({
  ...
  heading('tpl-conscar-culture-title', { x: 80, y: 40, width: 400, height: 50 }, '우리의 문화', 2, '#1f2a2e', 'left', 'tpl-conscar-culture'),
  createTextNode({
  ...
  ...buildConsultingProofSections('consulting-careers', STAGE_H),
]);
