...
const GRID_H = 620; // tightened from 700: section over-reserved, left a large empty gap
...
const cards: BuilderCanvasNode[] = cases.flatMap((c, i) => {
  ...
  return [
    ...
    heading(`${prefix}-t`, { x: 24, y: 24, width: 312, height: 36 }, c.title, 3, '#1f2a2e', 'left', prefix),
    createTextNode({
      id: `${prefix}-m`,
      parentId: prefix,
...
const nodes: BuilderCanvasNode[] = assignCanvasNodeZIndices([
  ...
  heading('tpl-conscase-grid-title', { x: 80, y: GRID_Y, width: 400, height: 50 }, '프로젝트 성과', 2, '#1f2a2e', 'left'),
  ...cards,
  ...buildConsultingProofSections('consulting-case-studies', STAGE_H),
]);
