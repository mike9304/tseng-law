...
vi.mock('@/lib/builder/ops/backups-store', () => ({
  listOpsBackups: vi.fn(),
}));

describe('GET /api/builder/ops/backups', () => {
  it('requires settings permission and short-circuits denied reads', async () => {
    ...
    const request = new NextRequest('https://law.example.test/api/builder/ops/backups');

    const response = await GET(request);

    expect(response.status).toBe(403);
    expect(guardBuilderReadWithPermission).toHaveBeenCalledWith(request, 'settings');
    expect(listOpsBackups).not.toHaveBeenCalled();
  });
});
