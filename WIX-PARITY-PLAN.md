...
| M158 | CMS | CMS collections/content manager v1 | F07~F16 | 24h | M157 | 🟢 editable CRUD + duplicate/search/sort + CSV + revisions + image fields/asset picker + permissions UI/gates + typecheck/unit/security passed |
| M159 | CMS | Dataset/repeater/dynamic page binding | F17~F26 | 32h | M158 | 🟡 home insights + services runtime datasets, generic repeater preview contract, scene/layers repeater preview nodes, selected-record dynamic URL + per-item SEO preview, service dataset inspector/E2E green, canvas service/FAQ item click focuses persisted collection editor, admin canvas service/FAQ repeater quick edit, dynamic template editor v0 block/record preview + draft save/load + publish slot / full visual repeater authoring + public dynamic-template renderer consumption remain |
