...
import { buildCategoryCtaBand, SUBPAGE_CTA_HEIGHT } from '../_shared/subpage-proof-sections';
...
function buildClassCard(item: ClassItem, idx: number): BuilderCanvasNode[] {
  ...
  return [
    ...
    heading(`${cid}-title`, { x: 16, y: 152, width: 228, height: 30 }, item.title, 3, '#14181f', 'left', cid),
    createTextNode({
      id: `${cid}-schedule`,
      parentId: cid,
...
const nodes: BuilderCanvasNode[] = assignCanvasNodeZIndices([
  ...
  ...buildCategoryCtaBand('fitness', 'fitness-classes', STAGE_H),
]);
...
export const fitnessClassesTemplate: PageTemplate = {
  ...
  document: {
    ...
    stageWidth: W,
    stageHeight: STAGE_H + SUBPAGE_CTA_HEIGHT,
    nodes,
  },
};
