...
function buildFaqItem(faq: Faq, idx: number): BuilderCanvasNode[] {
  ...
  return [
    createContainerNode({
      ...
      background: idx % 2 === 0 ? '#ffffff' : '#efe2cb',
      borderRadius: 10,
      borderColor: '#e0d3b8',
      borderWidth: 1,
      padding: 20,
    }),
    heading(`${cid}-q`, { x: 20, y: 16, width: CONTENT_W - 60, height: 32 }, faq.q, 3, '#2a2418', 'left', cid),
    createTextNode({
      id: `${cid}-a`,
      parentId: cid,
...
const nodes: BuilderCanvasNode[] = assignCanvasNodeZIndices([
  heading('tpl-edufaq-title', { x: MARGIN, y: 50, width: 600, height: 56 }, '입학 FAQ', 1, '#2a2418'),
  createTextNode({
  ...
  ...buildCategoryProofSections('education', 'education-faq', STAGE_H),
]);
