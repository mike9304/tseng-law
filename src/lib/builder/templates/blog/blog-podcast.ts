...
const CARD_H = 312; // +32 for 2-line titles
const CARD_GAP = 40;
...
function episodeCard(n: number, col: number, row: number): BuilderCanvasNode[] {
  ...
  return [
    ...
    heading(`${cId}-title`, { x: 220, y: 24, width: 296, height: 72 }, ep.title, 3, '#1a1a1a', 'left', cId),
    createTextNode({
      id: `${cId}-meta`,
      parentId: cId,
      rect: { x: 220, y: 100, width: 296, height: 24 },
    ...
    createTextNode({
      id: `${cId}-desc`,
      parentId: cId,
      rect: { x: 220, y: 136, width: 296, height: 80 },
    ...
    createContainerNode({
      id: `${cId}-play-area`,
      parentId: cId,
      rect: { x: 220, y: 232, width: 296, height: 48 },
      background: '#1a1a1a',
      borderRadius: 24,
      padding: 0,
    }),
...
const nodes: BuilderCanvasNode[] = assignCanvasNodeZIndices([
  ...
  ...episodeCard(3, 0, 1),
  ...episodeCard(4, 1, 1),
  ...buildCategoryProofSections('blog', 'blog-podcast', STAGE_H),
]);
