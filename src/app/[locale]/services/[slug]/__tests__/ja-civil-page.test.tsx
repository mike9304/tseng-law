...
describe('Japanese civil service-detail route', () => {
  beforeEach(() => {
  ...
  it('publishes exact Japanese metadata with canonical locale and all alternates', async () => {
    const approved = getJapaneseServiceDetail('civil');
    ...
    expect(metadata.alternates?.canonical).toBe(`${SITE_URL}/ja/services/civil`);
    expect(metadata.other?.['content-language']).toBe('ja');
    expect(metadata.openGraph).toMatchObject({
      title: approved!.title,
      description: expectedDescription,
      url: `${SITE_URL}/ja/services/civil`,
      locale: 'ja_JP',
    });
    expect(metadata.alternates?.languages).toEqual({
      ko: `${SITE_URL}/ko/services/civil`,
  ...
  it('renders exact approved body, ordered points, columns, links, and structured data', async () => {
    const approved = getJapaneseServiceDetail('civil');
    const base = getServiceArea('civil');
    ...
    expect(html).toContain(`${SITE_URL}/ja/services/civil`);
