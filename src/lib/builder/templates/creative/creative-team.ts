...
function buildMemberCard(member: TeamMember, idx: number): BuilderCanvasNode[] {
  ...
  return [
    ...
    heading(`${cid}-name`, { x: 20, y: 232, width: 220, height: 36 }, member.name, 3, '#0f0f12', 'center', cid),
    createTextNode({
      id: `${cid}-role`,
      parentId: cid,
...
const nodes: BuilderCanvasNode[] = assignCanvasNodeZIndices([
  heading('tpl-creativeteam-title', { x: MARGIN, y: 50, width: 500, height: 56 }, '팀 소개', 1, '#0f0f12'),
  createTextNode({
  ...
  ...buildCategoryProofSections('creative', 'creative-team', STAGE_H),
]);
