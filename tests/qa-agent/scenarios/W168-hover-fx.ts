...
// 호버(Hover) 섹션 — fade / rotate-3d / pulse(scale) / lift 프리셋.
const HOVER_PRESET_LABELS = ['호버 프리셋', 'Hover preset', 'Hover 預設'];
const TRANSITION_LABELS = ['전환', 'Transition'];
...
function hoverPresetCombo(page: Page) {
  return inspector(page).getByRole('combobox', { name: new RegExp(HOVER_PRESET_LABELS.join('|')) }).first();
}
...
export const W168_hoverFx: CheckpointDefinition = {
  id: 'W168',
  title: '호버 애니메이션: fade / rotate-3d / pulse(scale) 선택 → 적용 → undo',
  verification:
    '노드 선택 → 애니메이션 탭 → 호버 프리셋을 fade/rotate-3d/pulse/lift 로 변경 → 콤보박스 값 반영 + transition 컨트롤 존재 확인 → undo 복원',
  async run({ page, baseUrl, recordEvidence, log }) {
    ...
    if (!(await tabBtn.isVisible().catch(() => false))) {
      ...
      await recordEvidence('w168-anim-tab-missing');
      return { findings };
    }
    await tabBtn.click({ force: true });
    await page.waitForTimeout(350);

    const combo = hoverPresetCombo(page);
    if ((await combo.count().catch(() => 0)) === 0) {
      findings.push({ severity: 'blocker', summary: '호버 프리셋(Hover preset) 콤보박스를 찾을 수 없음' });
      await recordEvidence('w168-hover-combo-missing');
      return { findings };
    }

    const original = await combo.inputValue().catch(() => '');
    log(`현재 호버 프리셋: "${original}"`);

    const hoverSection = sectionOf(combo);
    // hover 섹션의 transition 컨트롤은 preset 과 무관하게 항상 활성(게이트 없음).
    const transitionInput = hoverSection
      .getByRole('spinbutton', { name: new RegExp(`${TRANSITION_LABELS.join('|')}.*\\(ms\\)|\\(ms\\)`) })
      .first();

    // fade(페이드), rotate-3d(회전), pulse(스케일 계열), lift 를 순회하며 commit 검증.
    for (const value of ['fade', 'rotate-3d', 'pulse', 'lift'] as const) {
      log(`호버 프리셋 → ${value}`);
      await combo.selectOption(value);
      await page.waitForTimeout(220);
      const applied = await combo.inputValue().catch(() => '');
      log(`적용 후 값="${applied}"`);
      await recordEvidence(`w168-${value}-applied`);
      if (applied !== value) {
        findings.push({
          severity: 'blocker',
          summary: `호버 프리셋이 ${value}(으)로 commit 되지 않음 (현재="${applied}")`,
        });
      }
    }

    // transition(전환 ms) 컨트롤이 존재하고 편집 가능한지 확인.
    const transitionCount = await transitionInput.count().catch(() => 0);
    if (transitionCount === 0) {
      findings.push({
        severity: 'visual',
        summary: '호버 섹션에 transition(전환 ms) 컨트롤이 렌더되지 않음',
      });
    } else {
      const transitionDisabled = await transitionInput.isDisabled().catch(() => true);
      log(`transition disabled=${transitionDisabled}`);
      if (transitionDisabled) {
        findings.push({ severity: 'visual', summary: '호버 transition(전환 ms) 컨트롤이 비활성 상태임' });
      }
    }

    log('undo 로 복원');
    // preset 변경 4회 누적 → undo 4회 + 수동 복원 보정.
    for (let i = 0; i < 4; i += 1) {
    ...
    if (restored !== original) {
      findings.push({
        severity: 'minor',
        summary: `undo 후 호버 프리셋이 원래값("${original}")으로 완전히 복원되지 않아 수동 복원함 (현재="${restored}")`,
      });
    }
    await recordEvidence('w168-undone');

    // Published-runtime emission 검증: fade 프리셋을 다시 적용해 발행하고 게시 페이지에
    // data-anim-hover="fade" 가 노출되는지 확인. 호버 런타임은 게시 페이지의
    // .builder-pub-node[data-anim-hover]:hover CSS 가 --builder-anim-hover-* 로 실행.
    log('fade 재적용 후 발행하여 게시 emission 확인');
    await combo.selectOption('fade');
    await page.waitForTimeout(220);
    const hoverPublished = await publishSiteViaApi(page, homePageId, log);
    if (!hoverPublished) {
      findings.push({
        severity: 'blocker',
        summary: '발행 플로우를 완료할 수 없어 게시 페이지의 hover(fade) emission 검증 불가',
      });
    } else {
      const { status, body } = await fetchPublishedBody(page, baseUrl);
      const hoverAttr = body.includes('data-anim-hover="fade"');
      log(`게시 페이지 status=${status}, data-anim-hover="fade"=${hoverAttr}`);
      await recordEvidence('w168-published-emission');
      if (hoverAttr) {
        log('게시 런타임 확인: .builder-pub-node[data-anim-hover="fade"]:hover CSS 가 opacity 전환 실행 (에디터 캔버스는 호버 시뮬레이션 없음)');
      } else {
        findings.push({
          severity: 'blocker',
          summary: '게시 페이지에 hover emission(data-anim-hover="fade")이 없음 — :hover CSS 룰이 노드에 연결되지 않음',
        });
      }
    }

    // 초안 복원 + 게시본 정리: 원래값으로 되돌리고 재발행하여 QA 효과가 home 에 남지 않도록 정리.
    log('초안 hover → 원래값 복원 후 재발행');
    await combo.selectOption(original || 'none').catch(() => undefined);
    await page.waitForTimeout(200);
    await publishSiteViaApi(page, homePageId, log).catch(() => undefined);
    await recordEvidence('w168-published-restored');
