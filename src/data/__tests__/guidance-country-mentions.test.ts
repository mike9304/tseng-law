...
describe('guidance data country-name gate', () => {
  ...
  it('scans every guidance locale block in every guidance data module', () => {
    const result = scanGuidanceCountryMentions();
    ...
    // vi/id/th/fil/ar cycle per record. Every record must still carry all five
    ...
    for (const entry of result.scannedFiles) {
      const cycle = ['vi', 'id', 'th', 'fil', 'ar', 'de', 'es', 'fr', 'pt', 'zh-hans', 'ms', 'ru', 'tr', 'it', 'nl', 'pl', 'hi', 'sv', 'da', 'nb', 'fi'] as const;
      ...
      if (entry.file.endsWith('international-guidance-western.ts')) {
        expect(entry.locales, `${entry.file} locale blocks`).toEqual(['de', 'es', 'fr', 'pt', 'ru', 'tr']);
        continue;
      }
      if (entry.file.endsWith('international-guidance-it-nl-pl.ts')) {
        expect(entry.locales, `${entry.file} locale blocks`).toEqual(['it', 'nl', 'pl']);
        continue;
      }
      if (entry.file.endsWith('international-guidance-asia.ts')) {
        expect(entry.locales, `${entry.file} locale blocks`).toEqual(['zh-hans', 'ms', 'hi']);
        continue;
      }
      if (entry.file.endsWith('international-guidance-nordic.ts')) {
