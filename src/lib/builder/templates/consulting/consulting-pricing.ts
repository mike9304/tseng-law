...
const tierCards: BuilderCanvasNode[] = tiers.flatMap((t, i) => {
  ...
  const bg = i === 1 ? '#1f2a2e' : '#eef1f0';
  const textColor = i === 1 ? '#ffffff' : '#1f2a2e';
  const titleColor = i === 1 ? '#ffffff' : '#1f2a2e';
  const priceColor = i === 1 ? '#3f5f5b' : '#3f5f5b';
  return [
    createContainerNode({
      id: prefix,
...
const nodes: BuilderCanvasNode[] = assignCanvasNodeZIndices([
  ...
  heading('tpl-consprc-tiers-title', { x: 80, y: TIERS_Y, width: 400, height: 50 }, '계약 모델', 2, '#1f2a2e', 'left'),
  ...tierCards,
  ...buildConsultingProofSections('consulting-pricing', STAGE_H),
]);
