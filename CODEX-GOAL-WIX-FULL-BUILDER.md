...
### 2026-05-13 scope correction

위 정량 기준은 "Wix visual editor parity layer"로 유지한다. 하지만 사용자의 목표는 전체 Wix이므로 W01~W225만으로는 완료 선언 금지.

추가 정량:
- `WIX-FULL-PRODUCT-GAP.md`의 F01~F120 중 자동 검증 + Codex 자평 기준 🟢 = 96 이상.
- M157~M176까지 CMS, App Market, Stores/eCommerce, AI, collaboration, developer platform, multilingual, enterprise/ops 영역을 닫아야 한다.
- 최종 종료 조건은 `W01~W225 203+` AND `F01~F120 96+` AND 사용자 OK 1회다.
