...
const cards: BuilderCanvasNode[] = services.flatMap((svc, i) => {
  ...
  return [
    ...
    heading(`${prefix}-t`, { x: 24, y: 24, width: 312, height: 36 }, svc.title, 3, '#2b2018', 'left', prefix),
    createTextNode({
      id: `${prefix}-d`,
      parentId: prefix,
...
const nodes: BuilderCanvasNode[] = assignCanvasNodeZIndices([
  ...
  heading('tpl-petsvc-grid-title', { x: 80, y: GRID_Y, width: 400, height: 50 }, '서비스 안내', 2, '#2b2018', 'left'),
  ...cards,
  ...buildCategoryProofSections('pet', 'pet-services', STAGE_H),
]);
