...
describe('Japanese IP service-detail route', () => {
  beforeEach(() => {
  ...
  it('publishes exact Japanese metadata with canonical locale and all alternates', async () => {
    const approved = getJapaneseServiceDetail('ip');
    ...
    expect(metadata.alternates?.canonical).toBe(`${SITE_URL}/ja/services/ip`);
    expect(metadata.other?.['content-language']).toBe('ja');
    expect(metadata.openGraph).toMatchObject({
      title: approved!.title,
      description: expectedDescription,
      url: `${SITE_URL}/ja/services/ip`,
      locale: 'ja_JP',
    });
    expect(metadata.alternates?.languages).toEqual({
      ko: `${SITE_URL}/ko/services/ip`,
  ...
  it('renders exact approved body, empty columns, links, and structured data', async () => {
    ...
    expect(html).toContain(`${SITE_URL}/ja/services/ip`);
