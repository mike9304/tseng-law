...
test.describe('/api/builder/translations/edit compact pipe rich text persistence', () => {
  ...
  test('preserves compact translated pipe separators outside adjacent rich text marks', async ({ request }) => {
    const token = `translation-rich-compact-pipe-${Date.now().toString(36)}`;
    const nodeId = `translation-rich-compact-pipe-node-${token}`;
