---
title: "Constituição de sociedades em Taiwan – parte aprofundada 3: procura do local"
url: "https://www.wei-wei-lawyer.com/post/taiwan-company-setup-pitch-location"
lastmod: "2026-09-10"
date_display: "13 de setembro de 2025"
read_time: "4 min de leitura"
categories:
  - "Constituição de sociedades em Taiwan"
featured_image: "../images/015-taiwan-company-setup-pitch-location/featured-01.jpg"
---

# Constituição de sociedades em Taiwan – parte aprofundada 3: procura do local

![Imagem de destaque](../images/015-taiwan-company-setup-pitch-location/featured-01.jpg)

(As exigências das autarquias locais de Taiwan diferem; o texto seguinte é um exemplo de abertura de um estabelecimento de restauração na cidade de Taipé.)

![](../images/015-taiwan-company-setup-pitch-location/img-01.jpg)

Quando se começa uma atividade de restauração, o primeiro problema é encontrar o endereço do local (營業地址).

Além da situação do mercado, há outro ponto importante.

Esse ponto é o seguinte.

Há que saber se esse endereço se situa numa zona onde a exploração de um «estabelecimento de restauração» é possível.

​

Se se começar a atividade na cidade de Taipé, há um instrumento público.

Pode utilizar-se o sistema de «consulta prévia do local» (營業場所預先查詢) do gabinete do comércio da cidade de Taipé (臺北市商業處).

<https://www.businesslocationinfo.gov.taipei/BLBQS/Home/Notice>

​

Pode verificar-se sem custos se, no endereço pretendido, a abertura de um «estabelecimento de restauração» é possível.

Contudo, para esta verificação, deve carregar-se o «extrato de inscrição imobiliária de tipo II» (建物登記第二類謄本, que contém as informações pormenorizadas do edifício) do endereço do local.

Qualquer pessoa pode pedir junto do gabinete da administração fundiária (地政事務所) o «extrato de inscrição imobiliária de tipo II» de um endereço qualquer.

Se tiver dificuldade em deslocar-se em pessoa ao gabinete da administração fundiária (地政事務所), há quem possa tratar do pedido.

Pode pedir a ajuda de um conhecido em Taiwan, do senhorio, de um agente imobiliário ou de uma advogada ou de um advogado.

Se o resultado da verificação indicar que o estabelecimento de restauração não é possível, mais tarde, no registo da sociedade (公司登記), a municipalidade poderia não aceitar a inscrição.

Preste portanto atenção a este ponto.

​

**Q. Todos os setores devem verificar, pelo sistema de «consulta prévia do local», se a exploração é possível?**

É recomendado a todos os setores confirmar, pelo sistema de «consulta prévia do local» (營業場所預先查詢), se a exploração é possível.

Contudo, na inscrição da sociedade, vários ramos são frequentemente inscritos de uma só vez, e deseja-se frequentemente inscrever cerca de 10 ramos de uma só vez.

Na prática, também não é necessário verificar todos os ramos na inscrição da sociedade.

Segundo as «[instruções de funcionamento do serviço de consulta assistida de locais de negócio da cidade de Taipé](https://www.laws.taipei.gov.tw/Law/LawSearch/LawArticleContent/FL080687)» (臺北市營業場所協助查詢服務作業須知) e o guia em vigor da cidade de Taipé, vigora uma regra de junção.

Desde 1 de janeiro de 2023, no pedido de inscrição de constituição, de transferência de sede ou de adição de ramos de atividade de uma sociedade ou de um comércio (incluindo a sucursal (分公司) e o estabelecimento auxiliar (分支機構)), o resultado da consulta prévia do local **deve** ser junto ao pedido de inscrição, independentemente do setor.

A lista abaixo dos «ramos sujeitos a consulta por iniciativa do serviço» (主動查詢之營業項目) designa os ramos que o gabinete do comércio consulta por iniciativa própria durante o exame da inscrição (隨案主動查詢) quando o ramo figura no pedido mas falta no resultado junto; o facto de um ramo não figurar nesta lista não significa que a verificação seja dispensada.

Se se abrir num lugar onde o estabelecimento de restauração não é possível, uma coima administrativa (罰鍰) da autoridade competente pode mais tarde ser infligida.

![«ramos sujeitos a consulta por iniciativa do serviço» (主動查詢之營業項目)](../images/015-taiwan-company-setup-pitch-location/img-02.jpg)

«ramos sujeitos a consulta por iniciativa do serviço» (主動查詢之營業項目)

Tal é o essencial; para outras perguntas, pode escrever através da página Contacto; não é prometido um prazo de resposta.

As regras das autoridades administrativas podem mudar com frequência; antes do registo da sociedade, convém confirmar a regulamentação mais recente.

---

> Ver também:
> - [Assuntos que tratamos](/pt/services)
> - [Equipa internacional de Hovering](/pt/lawyers)
> - [Como contactar o escritório](/pt/contact)
