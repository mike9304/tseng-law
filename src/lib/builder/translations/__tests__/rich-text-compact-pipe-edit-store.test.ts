...
describe('applyTranslationToLocaleDraft compact pipe rich text mark boundaries', () => {
  ...
  it('keeps compact pipe separators outside adjacent link and underline marks', async () => {
    ...
    if (node?.kind !== 'text') throw new Error('translated compact pipe text node was not written');
