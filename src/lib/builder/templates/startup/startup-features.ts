...
const cards: BuilderCanvasNode[] = features.flatMap((f, i) => {
  ...
  return [
    ...
    heading(`${prefix}-t`, { x: 24, y: 24, width: 312, height: 36 }, f.title, 3, '#16191f', 'left', prefix),
    createTextNode({
      id: `${prefix}-d`,
      parentId: prefix,
...
const nodes: BuilderCanvasNode[] = assignCanvasNodeZIndices([
  ...
  heading('tpl-stupfeat-grid-title', { x: 80, y: GRID_Y, width: 400, height: 50 }, '주요 기능', 2, '#16191f', 'left'),
  ...cards,
  ...buildCategoryProofSections('startup', 'startup-features', STAGE_H),
]);
