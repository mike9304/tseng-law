...
type GlobalFooterDraftRequestBody = {
...
export async function GET(request: NextRequest) {
  ...
  const draft = await readFooterCanvas(siteId);
  if (!draft) {
    return errorResponse(locale, 'global_footer_draft_not_found', 404);
  }
...
export async function PUT(request: NextRequest) {
  ...
  let body: GlobalFooterDraftRequestBody;
  const locale = normalizeLocale(request.nextUrl.searchParams.get('locale') || 'ko');
  try {
    body = (await request.json()) as GlobalFooterDraftRequestBody;
  } catch {
  ...
  await writeFooterCanvas(siteId, normalized);
