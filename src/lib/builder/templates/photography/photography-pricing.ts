...
function packageCard(n: number): BuilderCanvasNode[] {
  ...
  const bg = n === 2 ? '#1a1a1a' : '#ece9e3';
  const textColor = n === 2 ? '#ffffff' : '#1a1a1a';
  const titleColor = n === 2 ? '#9b8463' : '#1a1a1a';
  return [
    createContainerNode({
      id: cId,
...
const nodes: BuilderCanvasNode[] = assignCanvasNodeZIndices([
  ...
  ...buildCategoryProofSections('photography', 'photography-pricing', STAGE_H),
]);
