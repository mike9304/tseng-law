...
const DRINKS_H = 250; // tightened from 400: section over-reserved, left a large empty gap
const FOOD_Y = DRINKS_Y + DRINKS_H + 80;
const FOOD_H = 250; // tightened from 400: section over-reserved, left a large empty gap
...
const drinkCards: BuilderCanvasNode[] = drinks.flatMap((d, i) => {
  ...
  return [
    ...
    heading(`${prefix}-t`, { x: 24, y: 24, width: 312, height: 36 }, d.name, 3, '#1f2622', 'left', prefix),
    createTextNode({
      id: `${prefix}-items`,
      parentId: prefix,
...
const foodCards: BuilderCanvasNode[] = foods.flatMap((f, i) => {
  ...
  return [
    ...
    heading(`${prefix}-t`, { x: 24, y: 24, width: 312, height: 36 }, f.name, 3, '#1f2622', 'left', prefix),
    createTextNode({
      id: `${prefix}-items`,
      parentId: prefix,
      rect: { x: 24, y: 66, width: 312, height: 70 },
...
const nodes: BuilderCanvasNode[] = assignCanvasNodeZIndices([
  ...
  /* ── Drinks ─────────────────────────────────────────────── */
  heading('tpl-cafemenu-drinks-title', { x: 80, y: DRINKS_Y, width: 400, height: 50 }, '음료 메뉴', 2, '#1f2622', 'left'),
  ...drinkCards,

  /* ── Food ────────────────────────────────────────────────── */
  heading('tpl-cafemenu-food-title', { x: 80, y: FOOD_Y, width: 400, height: 50 }, '푸드 메뉴', 2, '#1f2622', 'left'),
  ...foodCards,
  ...buildCategoryProofSections('cafe', 'cafe-menu', STAGE_H),
]);
