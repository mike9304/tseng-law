...
describe('Japanese traffic column 003 — Q6–Q10 translation contract', () => {
  ...
  it('makes every Q7 damages category conditional under Civil Code Articles 184 and 216', () => {
    ...
    expect(q7).toMatch(
      /(?:就労不能|働けない).{0,40}(?:収入減少|所得減少|逸失収入).{0,60}(?:労働能力|稼働能力).{0,20}(?:低下|減少|喪失).{0,60}(?:第)?195条.{0,40}非財産的損害/su,
    );
    expect(q7).toMatch(
  ...
  it('requires complete Q9 care proof and treats unpaid family care as non-automatic', () => {
    ...
    expect(q9).toMatch(
      /事故.{0,20}因果関係.{0,50}介護.{0,20}必要性.{0,50}(?:実際の提供|実際に提供|実際に介護が行われた).{0,50}期間.{0,50}(?:合理的|相当).{0,20}(?:金額|額)/su,
    );
    expect(q9).toMatch(
  ...
  it('connects Q10 travel proof to treatment and rejects automatic taxi-receipt sufficiency', () => {
    expect(q10).toMatch(
      /交通費.{0,40}治療記録.{0,60}事故.{0,20}(?:負傷|傷害).{0,30}(?:関連|結び付き)/su,
    );
    expect(q10).toMatch(
...
describe('Japanese traffic column 003 — Q11–Q15 translation contract', () => {
  ...
  it('requires accident-caused inability and actual income reduction while preserving Q12 as a separate issue', () => {
    expect(q11).toMatch(
      /事故.{0,25}(?:負傷|傷害).{0,100}(?:全部|完全).{0,20}(?:または|若しくは|・).{0,20}(?:一部|部分).{0,50}(?:就労|働くこと).{0,30}(?:不能|不可能|できな)/su,
    );
    expect(q11).toMatch(
      /治療.{0,10}回復期間.{0,100}(?:現実|実際).{0,20}(?:収入|所得).{0,15}(?:減少|低下).{0,30}(?:資料|証拠).{0,20}(?:裏付け|立証)/su,
    );
    expect(q11).toMatch(
      /(?:診断書|休養の勧告).{0,100}(?:関連|有用|重要).{0,50}(?:それだけ|単独).{0,40}(?:決まら|十分では|結論にはなら|認められるわけでは)/su,
    );
  ...
  it('treats lasting earning-capacity loss as individualized rather than mechanically fixed in Q12', () => {
    ...
    expect(q12).toMatch(
      /(?:現在の)?給与.{0,30}(?:変わらない|同じ|維持).{0,70}(?:当然|自動的|直ちに).{0,30}(?:排除|否定).{0,20}(?:されない|されるわけでは(?:ない|ありません)|するものでは(?:ない|ありません))/su,
    );
    expect(q12).toMatch(
      /(?:障害率|機能障害率).{0,60}(?:現在の)?給与.{0,70}(?:機械的|自動的).{0,35}(?:決まら|決まるわけ|定ま|算定され)/su,
    );
    expect(q12).toMatch(
      /(?:争い|争われ).{0,80}医学的鑑定.{0,80}(?:有用|役立つ).{0,80}(?:すべて|全て).{0,25}(?:必須|必要).{0,20}(?:ではない|ではありません|とは限らない)/su,
    );
    expect(q12).toMatch(
    ...
    expect(q12).toMatch(
      /ホフマン.{0,40}(?:計算機|計算).{0,50}(?:補助|参考).{0,100}(?:必須|義務).{0,30}(?:ではない|ではなく|でもなく).{0,60}(?:保証|確約).{0,20}(?:しない|しません|ではない)/su,
    );
    expect(q12).toMatch(
  ...
  it('states the current compulsory benefits and treats all three other Q15 products as policy-dependent', () => {
    expect(q15).toMatch(
      /強制汽車責任保險法.{0,20}(?:第)?6条.{0,100}(?:車両|自動車).{0,25}所有者.{0,80}(?:使用者|利用者|使用人).{0,20}(?:または|若しくは|・).{0,20}管理者/su,
    );
    expect(q15).toMatch(
      /(?:無過失|過失の有無を問わず).{0,100}(?:負傷|傷害).{0,25}(?:死亡|死者).{0,100}同乗者.{0,80}(?:車外|車両外).{0,20}(?:第三者|第三者である者)/su,
    );
    expect(q15).toMatch(
      /単独車両事故.{0,100}運転者.{0,80}(?:当該|その|自分の).{0,25}車両.{0,30}強制保険.{0,35}(?:対象外|(?:給付)?対象では(?:ない|ありません)|補償されない)/su,
    );
    expect(q15).toMatch(
