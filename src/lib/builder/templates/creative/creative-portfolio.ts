...
function buildProjectCard(proj: Project, idx: number): BuilderCanvasNode[] {
  ...
  return [
    ...
    heading(`${cid}-title`, { x: 20, y: 238, width: 310, height: 36 }, proj.title, 3, '#0f0f12', 'left', cid),
    createTextNode({
      id: `${cid}-desc`,
      parentId: cid,
...
const nodes: BuilderCanvasNode[] = assignCanvasNodeZIndices([
  heading('tpl-creativeport-title', { x: MARGIN, y: 50, width: 500, height: 56 }, '포트폴리오', 1, '#0f0f12'),
  createTextNode({
  ...
  ...buildCategoryProofSections('creative', 'creative-portfolio', STAGE_H),
]);
