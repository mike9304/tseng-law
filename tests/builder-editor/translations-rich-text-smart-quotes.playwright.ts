...
test.describe('/api/builder/translations/edit smart quote rich text persistence', () => {
  ...
  test('preserves translated curly quotes outside adjacent rich text marks', async ({ request }) => {
    const token = `translation-rich-smart-quotes-${Date.now().toString(36)}`;
    const nodeId = `translation-rich-smart-quotes-node-${token}`;
