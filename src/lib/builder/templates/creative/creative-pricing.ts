...
function buildTierCard(tier: PricingTier, idx: number): BuilderCanvasNode[] {
  ...
  return [
    createContainerNode({
      ...
      background: tier.featured ? '#0f0f12' : '#ffffff',
      borderRadius: 12,
      borderColor: tier.featured ? '#0f0f12' : '#e6e6e9',
      borderWidth: 1,
      padding: 32,
    }),
    heading(
    ...
    createTextNode({
      ...
      text: tier.features,
      fontSize: 14,
      color: tier.featured ? 'rgba(255,255,255,0.85)' : '#0f0f12',
      lineHeight: 1.8,
      align: 'center',
    }),
    createButtonNode({
      ...
      style: { backgroundColor: tier.featured ? '#e0457a' : 'transparent', borderRadius: 6 },
    }),
  ];
}

const nodes: BuilderCanvasNode[] = assignCanvasNodeZIndices([
  heading('tpl-creativepricing-title', { x: MARGIN, y: 50, width: 500, height: 56 }, '가격 안내', 1, '#0f0f12'),
  createTextNode({
  ...
  ...buildCategoryContactMethods('creative', 'creative-pricing-dir', STAGE_H),
  ...buildCategoryProofSections('creative', 'creative-pricing', STAGE_H + CONTACT_METHODS_HEIGHT),
]);
